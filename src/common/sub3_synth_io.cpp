...
void sub3_synth::increment_patch(int category,int patch)
{
	if (category)
	{
		...
		if(current_category_id >= n) current_category_id = 0;
		else if(current_category_id < 0) current_category_id = n-1;
		n = storage.patch_list.size();
		for(int i=0; i<n; i++)
		{
			if (storage.patch_list[i].category == current_category_id)	// find the first patch within the new category
			{
				//load_patch(i);
				patchid_queue = i; 
	...
	else
	{
		...
		if(patchid_queue < 0) patchid_queue = storage.patch_list.size()-1;
		process_threadunsafe_operations();
		return;
	}
}

void sub3_synth::load_patch(int id)
{	
	if (id<0) id = storage.patch_list.size()-1;
	else if (id >= storage.patch_list.size()) id = 0;
	patchid = id;
	if(id >= storage.patch_list.size()) return;
	...
	if((vt_read_int32BE(fxp.chunkMagic) != 'CcnK')||(vt_read_int32BE(fxp.fxMagic) != 'FPCh')||(vt_read_int32BE(fxp.fxID) != 'cjs3')) { fclose(f); return; }
	...
	int error = ferror(f);
	if(error) perror("error while loading patch");
	fclose(f);
...
/*	AUPreset preset;
	...
	preset.presetName = CFStringCreateWithCString(NULL,storage.patch_list[patchid].name.c_str(), kCFStringEncodingUTF8);
	((aulayer*)parent)->SetAFactoryPresetAsCurrent(preset);*/
	#endif
	
}

void sub3_synth::load_raw(const void *data,int size, bool preset)
{
	halt_engine = true;
	all_notes_off();
	for(int s=0; s<2; s++) for(int i=0; i<n_customcontrollers; i++) storage.getPatch().scene[s].modsources[ms_ctrl1+i]->reset();
	...
	setParameter01(storage.getPatch().scene[0].f2_cutoff_is_offset.id,storage.getPatch().scene[0].f2_cutoff_is_offset.get_value_f01());
	setParameter01(storage.getPatch().scene[1].f2_cutoff_is_offset.id,storage.getPatch().scene[1].f2_cutoff_is_offset.get_value_f01());	
...
string sub3_synth::getUserPatchDirectory()
{
   return storage.userDataPath;
}
string sub3_synth::getLegacyUserPatchDirectory()
{
#if MAC
...
bool askIfShouldOverwrite()
{
   ...
   CFUserNotificationDisplayAlert(0, kCFUserNotificationPlainAlertLevel, 0, 0, 0, CFSTR("Overwrite?"),
      CFSTR("The file already exist, do you wish to overwrite it?"), CFSTR("Yes"), CFSTR("No"),
      0, &responseFlags);
   if ((responseFlags & 0x3) != kCFUserNotificationDefaultResponse) return false;
#else
   if (MessageBox(::GetActiveWindow(), "The file already exist, do you wish to overwrite it?", "Overwrite?", MB_YESNO | MB_ICONQUESTION) != IDYES) return false;
#endif

   return true;
}

void sub3_synth::save_patch()
{
	if(storage.getPatch().category.empty()) storage.getPatch().category = "default";
   ...
   filename.append(legalname + ".fxp");
	...
	{
      if (!askIfShouldOverwrite()) return;
	}
...
unsigned int sub3_synth::save_raw(void **data)
{
