...
LossFilter::LossFilter (SurgeStorage *storage, size_t order) : order (order),
    bumpFilter (storage)
{
...
void LossFilter::calcCoefs()
{
    ...
    for (int n = 0; n < curOrder / 2; n++)
    {
        ...
        for (int k = 0; k < curOrder; k++)
            currentCoefs[idx] += HCoefs[k] * std::cos (2.0f * M_PI * (float) k * (float) n / (float) curOrder);
...
{
    if ((curSpeed != prevSpeed || curSpacing != prevSpacing || curThick != prevThick || curGap != prevGap) && fadeCount == 0)
    {
        calcCoefs();
