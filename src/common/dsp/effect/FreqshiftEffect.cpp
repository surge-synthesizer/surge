...
void FreqshiftEffect::setvars(bool init)
{
   ...
   if (init)
      time.newValue((fxdata->p[freq_delay].temposync ? storage->temposyncratio_inv : 1.f) *
                    samplerate * storage->note_to_pitch_ignoring_tuning(12 * fxdata->p[freq_delay].val.f) - FIRoffset);
   else
      time.newValue((fxdata->p[freq_delay].temposync ? storage->temposyncratio_inv : 1.f) *
                     samplerate * storage->note_to_pitch_ignoring_tuning(12 * *f[freq_delay]) - FIRoffset);
