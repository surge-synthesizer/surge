...
{
   const __m128 hb_c = _mm_set1_ps(0.5f); // om denna ändras från 0.5, se till att ändra i koden
														// eftersom det antas att den är hälften
														// TRANSLATE:
														// If this is changed from 0.5, make sure to change 
   ...
   case fb_serial: // no feedback at all  (saves CPU)
      // TODO denna skulle nog kunna bli ännu snabbare om den serialiseras (vilket är lättare nu)
      // lite mer L1 vänlig, men latency-lalal blir nog detsamma
		// TRANSLATE: 
		// TODO this might be even faster if it is serialized (which is now easier)
		// a little more L1 friendly, but latency-lalal will probably be the same
