...
template <bool is_init> void WavetableOscillator::update_lagvals()
{
   ...
   float hpf2 =
       min(integrator_hpf, powf(hpf_cycle_loss, 4 * invt)); // TODO ACHTUNG! WARNING! Make a lookup table (gör=make?)
    ...
    float pitch0, float drift, bool stereo, bool FM, float depth)
{
   ...
   if ((oscdata->wt.n_tables == 1) ||
       (tableid >= oscdata->wt.n_tables)) // TableID-range may have changed in the meantime, check it!
   {
      tableipol = 0.f;
      tableid = 0;
      last_tableid = 0;
