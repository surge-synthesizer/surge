...
reverb1::reverb1(sub3_storage *storage, sub3_fx *fxdata, pdata* pd) 
: baseeffect(storage,fxdata,pd), band1(storage), locut(storage), hicut(storage)
{	
	b = 0;	
}
...
{		
	...
	band1.coeff_peakEQ(band1.calc_omega(fxdata->p[rp_freq1].val.f/12.f),2,fxdata->p[rp_gain1].val.f);
...
void reverb1::setvars(bool init)
{	
}

void reverb1::clear_buffers()
{	
	clear_block(predelay,max_rev_dly>>2);
	clear_block(delay,(rev_taps*max_rev_dly)>>2);
}

void reverb1::loadpreset(int id)
{
	shape = id;
...
{	
	...
	float t = block_size_inv * ((float)(max_dt>>8) + samplerate * powf(2.f,*f[rp_decaytime]) * 2.f);	// *2 is to get the db120 time
	ringout_time = (int)t;
}
void reverb1::update_rsize()
{
...
void reverb1::process(float *dataL, float *dataR)
{				
	_MM_ALIGN16 float wetL[block_size],wetR[block_size];	
	
	if(fxdata->p[rp_shape].val.i != shape) loadpreset(fxdata->p[rp_shape].val.i);
	if((b == 0)&&(fabs(*f[rp_roomsize] - lastf[rp_roomsize]) > 0.001f)) loadpreset(shape);
//	if(fabs(*f[rp_variation] - lastf[rp_variation]) > 0.001f) update_rsize();
	if(fabs(*f[rp_decaytime] - lastf[rp_decaytime]) > 0.001f) update_rtime();		
	...
	int pdtime = (int)(float)samplerate * note_to_pitch(12* *f[rp_predelay]) * (fxdata->p[rp_predelay].temposync?storage->temposyncratio_inv:1.f);	
...
void reverb1::suspend()
{ 
	init();
}

const char* reverb1::group_label(int id)
{
	switch (id)
	{
	case 0:
...
int reverb1::group_label_ypos(int id)
{
	switch (id)
	{
	case 0:
...
void reverb1::init_ctrltypes()
{
	baseeffect::init_ctrltypes();

	fxdata->p[rp_predelay].set_name("Pre-Delay");	fxdata->p[rp_predelay].set_type(ct_envtime);		fxdata->p[rp_predelay].modulateable = false;	
	
	fxdata->p[rp_shape].set_name("Room Shape");		fxdata->p[rp_shape].set_type(ct_reverbshape);	fxdata->p[rp_shape].modulateable = false;		
	fxdata->p[rp_roomsize].set_name("Size");		fxdata->p[rp_roomsize].set_type(ct_percent);		fxdata->p[rp_roomsize].modulateable = false;
	fxdata->p[rp_decaytime].set_name("Decay Time");	fxdata->p[rp_decaytime].set_type(ct_reverbtime);	
	fxdata->p[rp_damping].set_name("HF Damp");		fxdata->p[rp_damping].set_type(ct_percent);			

	fxdata->p[rp_locut].set_name("Low Cut");		fxdata->p[rp_locut].set_type(ct_freq_audible);		
	fxdata->p[rp_freq1].set_name("Band1 Freq");		fxdata->p[rp_freq1].set_type(ct_freq_audible);		
	fxdata->p[rp_gain1].set_name("Band1 Gain");		fxdata->p[rp_gain1].set_type(ct_decibel);			
	fxdata->p[rp_hicut].set_name("High Cut");		fxdata->p[rp_hicut].set_type(ct_freq_audible);		
	
	fxdata->p[rp_mix].set_name("Mix");				fxdata->p[rp_mix].set_type(ct_percent);	
	fxdata->p[rp_width].set_name("Width");			fxdata->p[rp_width].set_type(ct_decibel_narrow);

	

	
	//fxdata->p[rp_moddepth].set_name("mod depth");	fxdata->p[rp_moddepth].set_type(ct_percent);		//fxdata->p[rp_modrate].set_name("mod rate");		fxdata->p[rp_modrate].set_type(ct_lforate);		
	//fxdata->p[rp_variation].set_name("irregularity");	fxdata->p[rp_variation].set_type(ct_percent);	
...
void reverb1::init_default_values()
{
