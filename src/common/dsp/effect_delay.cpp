...
dualdelay::dualdelay(sub3_storage *storage, sub3_fx *fxdata, pdata* pd) 
: baseeffect(storage,fxdata,pd), timeL(0.0001), timeR(0.0001),
...
void dualdelay::setvars(bool init)
{
	...
	float lforate = envelope_rate_linear(-*f[6]) * (fxdata->p[6].temposync?storage->temposyncratio:1.f);
	lfophase += lforate;
	if(lfophase>0.5) 
	{
		lfophase -= 1;
	...
	if (LFOdirection) LFOval = ca*LFOval + lfo_increment;
	else LFOval = ca*LFOval - lfo_increment;
	
	if(init)
	{
		timeL.newValue(samplerate * ((fxdata->p[0].temposync?storage->temposyncratio_inv:1.f) * note_to_pitch(12 * fxdata->p[0].val.f)) + LFOval - FIRoffset);			
		timeR.newValue(samplerate * ((fxdata->p[1].temposync?storage->temposyncratio_inv:1.f) * note_to_pitch(12 * fxdata->p[1].val.f)) - LFOval - FIRoffset);	
	}
	else
	{
		timeL.newValue(samplerate * ((fxdata->p[0].temposync?storage->temposyncratio_inv:1.f) * note_to_pitch(12* *f[0])) + LFOval - FIRoffset);			
		timeR.newValue(samplerate * ((fxdata->p[1].temposync?storage->temposyncratio_inv:1.f) * note_to_pitch(12* *f[1])) - LFOval - FIRoffset);	
	}
...
void dualdelay::process(float *dataL, float *dataR)
{			
	setvars(false);
...
const char* dualdelay::group_label(int id)
{
	switch (id)
	{
	case 0:
...
int dualdelay::group_label_ypos(int id)
{
	switch (id)
	{
	case 0:
...
void dualdelay::init_ctrltypes()
{
	...
	//fxdata->p[8].set_name("ducking");			fxdata->p[8].set_type(ct_decibel_extra_narrow);
	//fxdata->p[9].set_name("rate");				fxdata->p[9].set_type(ct_envtime);	
	...
	fxdata->p[11].set_name("Width");			fxdata->p[11].set_type(ct_decibel_narrow);	
