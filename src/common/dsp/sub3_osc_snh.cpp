...
osc_snh::osc_snh(sub3_storage *storage, sub3_osc *oscdata, pdata *localcopy) : oscillator_BLIT(storage,oscdata,localcopy)
{	
}
...
void osc_snh::init(float pitch, bool is_display)
{
	assert(storage);
	first_run = true;	
#if PPC
...
void osc_snh::init_ctrltypes()
{
	...
	oscdata->p[2].set_name("-");			oscdata->p[2].set_type(ct_none);
	oscdata->p[3].set_name("-");			oscdata->p[3].set_type(ct_none);
...
void osc_snh::init_default_values()
{
...
void osc_snh::convolute(int voice, bool FM, bool stereo)
{
	...
	if (syncstate[voice]<oscstate[voice])
	{
		...
		if (state[voice]==1) invertcorrelation = -1.f;
		state[voice] = 0;								
...
void osc_snh::process_block(float pitch0, float drift, bool stereo, bool FM, float depth)
{	
	...
	{				
		...
		{	
			...
			while((syncstate[l] < a)||(oscstate[l] < a)) convolute(l,false,stereo);
