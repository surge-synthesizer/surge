...
baseeffect::baseeffect(sub3_storage *storage, sub3_fx *fxdata, pdata* pd)
{
	...
	this->pd = pd;
	ringout = 10000000;
	for(int i=0; i<n_fx_params; i++) f[i] = &pd[fxdata->p[i].id].f;
}

bool baseeffect::process_ringout(float *dataL, float *dataR, bool indata_present)
{ 
	if(indata_present) ringout = 0;
	else ringout++;
...
{	
	if(init)
	{
		...
		gain.instantize();
		band1.coeff_peakEQ(band1.calc_omega(fxdata->p[1].val.f*(1.f/12.f)), fxdata->p[2].val.f, 1.f);	// sätt banden till 0dB så fades eqn in
		band2.coeff_peakEQ(band2.calc_omega(fxdata->p[4].val.f*(1.f/12.f)), fxdata->p[5].val.f, 1.f);
		band3.coeff_peakEQ(band3.calc_omega(fxdata->p[7].val.f*(1.f/12.f)), fxdata->p[8].val.f, 1.f);
...
void eq3band::init_ctrltypes()
{
	...
	fxdata->p[0].set_name("L Gain");		fxdata->p[0].set_type(ct_decibel);
	fxdata->p[1].set_name("L Freq");		fxdata->p[1].set_type(ct_freq_audible);
	fxdata->p[2].set_name("L BW");		fxdata->p[2].set_type(ct_bandwidth);		

	fxdata->p[3].set_name("M Gain");		fxdata->p[3].set_type(ct_decibel);
	fxdata->p[4].set_name("M Freq");		fxdata->p[4].set_type(ct_freq_audible);
	fxdata->p[5].set_name("M BW");		fxdata->p[5].set_type(ct_bandwidth);		

	fxdata->p[6].set_name("H Gain");		fxdata->p[6].set_type(ct_decibel);
	fxdata->p[7].set_name("H Freq");		fxdata->p[7].set_type(ct_freq_audible);
	fxdata->p[8].set_name("H BW");		fxdata->p[8].set_type(ct_bandwidth);		

	fxdata->p[9].set_name("Gain");		fxdata->p[9].set_type(ct_decibel);		

	for(int i=0; i<10; i++) fxdata->p[i].posy_offset = 1 + 2*(i/3);
}
...
template<int v>
void chorus<v>::setvars(bool init)
{
	if(init)
	{
	...
	else
	{
		...
		float rate = envelope_rate_linear(-*f[1]) * (fxdata->p[1].temposync?storage->temposyncratio:1.f);
		float tm = note_to_pitch(12* *f[0]) * (fxdata->p[0].temposync?storage->temposyncratio_inv:1.f);
		for(int i=0; i<v; i++)
		{
			lfophase[i] += rate;
...
template<int v>
void chorus<v>::process(float *dataL, float *dataR)
{			
	setvars(false);
	...
	{						
		...
		{	
			...
			vo = _mm_add_ps(vo,_mm_mul_ps(_mm_load_ps(&sinctable1X[sinc+4]),_mm_loadu_ps(&buffer[rp+4])));
			vo = _mm_add_ps(vo,_mm_mul_ps(_mm_load_ps(&sinctable1X[sinc+8]),_mm_loadu_ps(&buffer[rp+8])));
...
template<int v>
const char* chorus<v>::group_label(int id)
{
	switch (id)
	{
	case 0:
