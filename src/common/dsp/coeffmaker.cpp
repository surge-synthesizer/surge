...
void coeffmaker::MakeCoeffs(float Freq, float Reso, int Type, int SubType)
{			
	switch(Type)
	{
	case fut_lp12:
		if(SubType==st_SVF) Coeff_SVF(Freq,Reso,false);
		else Coeff_LP12(Freq,Reso,SubType);
		break;
	case fut_hp12:
		if(SubType==st_SVF) Coeff_SVF(Freq,Reso,false);
		else Coeff_HP12(Freq,Reso,SubType);
		break;
	case fut_bp12:
		if((SubType==st_SVF)||(SubType==3)) Coeff_SVF(Freq,Reso,false);		
		else Coeff_BP12(Freq,Reso,SubType);
		break;
	case fut_br12:
		Coeff_BR12(Freq,Reso,SubType);
		break;
	case fut_lp24:
		if(SubType==st_SVF) Coeff_SVF(Freq,Reso,true);
		else Coeff_LP24(Freq,Reso,SubType);
		break;
	case fut_hp24:
		if(SubType==st_SVF) Coeff_SVF(Freq,Reso,true);
		else Coeff_HP24(Freq,Reso,SubType);
		break;
	case fut_lpmoog:
...
#define boundfreq(freq) { if (freq>75) freq = 75; if(freq < -55) freq = -55; }
...
void coeffmaker::Coeff_SVF(float Freq, float Reso, bool FourPole)
{
...
void coeffmaker::Coeff_LP12(float freq, float reso, int subtype)
{					
	...
	if(subtype != st_Smooth) alpha = min(alpha, sqrt(1.0-cosi*cosi) - 0.0001);
				
	double	b0 = (1 - cosi)*0.5,
			b1 = 1 - cosi,
			b2 = (1 - cosi)*0.5,		
			a0 =  1 + alpha,
			a1 = -2*cosi,
	...
	if(subtype == st_Smooth) ToNormalizedLattice(a0inv,a1,a2,b0*gain,b1*gain,b2*gain,clipscale(freq,subtype));	
	else ToCoupledForm(a0inv,a1,a2,b0*gain,b1*gain,b2*gain,clipscale(freq,subtype));	
}

void coeffmaker::Coeff_LP24(float freq, float reso, int subtype)
{			
	float gain = resoscale(reso,subtype);
...
void coeffmaker::Coeff_HP12(float freq, float reso, int subtype)
{			
	float gain = resoscale(reso,subtype);
	...
	double	alpha = sinu*Q2inv;
	if(subtype != 0) alpha = min(alpha, sqrt(1.0-cosi*cosi) - 0.0001);
...
void coeffmaker::Coeff_HP24(float freq, float reso, int subtype)
{		
	float gain = resoscale(reso,subtype);
...
void coeffmaker::Coeff_BP12(float freq, float reso, int subtype)
{			
	float gain = resoscale(reso,subtype);
	if(subtype == 1) gain *= 2.f;
...
void coeffmaker::Coeff_BR12(float freq, float reso, int subtype)
{			
	boundfreq(freq)
...
void coeffmaker::Coeff_LP4L(float freq, float reso, int subtype)
{
	double gg = limit_range(((double)440*note_to_pitch(freq)*dsamplerate_os_inv),0.0,0.187); // gg
...
void coeffmaker::Coeff_COMB(float freq, float reso, int subtype)
{		
	float dtime = (1.f/440.f)*note_to_pitch(-freq);
	dtime = dtime*dsamplerate_os - FIRoffset; // 1 sample f�r feedback, 1 f�r ett IIR-filter utan resonans
...
void coeffmaker::Coeff_SNH(float freq, float reso, int subtype)
{		
...
void coeffmaker::FromDirect(float N[n_cm_coeffs])
{
	if(FirstRun)
	{				
...
void coeffmaker::ToNormalizedLattice(double a0inv,double a1,double a2,double b0,double b1,double b2,double g)
{	
...
void coeffmaker::ToCoupledForm(double a0inv,double a1,double a2,double b0,double b1,double b2,double g)
{	
	b0 *= a0inv;	b1 *= a0inv;	b2 *= a0inv;	a1 *= a0inv;	a2 *= a0inv;					
