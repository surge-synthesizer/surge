...
float sub3_voice_state::getPitch()
{
   return key + mainChannelState->pitchBendInSemitones + voiceChannelState->pitchBendInSemitones + detune;
}

sub3_voice::sub3_voice(
...
{	
   ...
   state.voiceChannelState = voiceChannelState;
	if((scene->polymode.val.i == pm_mono_st_fp) || (scene->portamento.val.f == scene->portamento.val_min.f))
	...
	for(int i=0; i<6; i++)
	{
		lfo[i].assign(storage,&scene->lfo[i],localcopy,&state,&storage->getPatch().stepsequences[state.scene_id][i]);		
		modsources[ms_lfo1+i] = &lfo[i];
	}	
	modsources[ms_velocity] = new modulation_source();
	modsources[ms_keytrack] = new modulation_source();	
	modsources[ms_polyaftertouch] = new modulation_controller();
	((modulation_controller*)modsources[ms_polyaftertouch])->init(storage->poly_aftertouch[state.scene_id&1][state.key&127]);	
	modsources[ms_velocity]->output = state.fvel;
	modsources[ms_keytrack]->output = 0;
	modsources[ms_ampeg] = new envelope_adsr(storage,&scene->adsr[0],localcopy,&state);
	modsources[ms_filtereg] = new envelope_adsr(storage,&scene->adsr[1],localcopy,&state);	

	modsources[ms_modwheel] = oscene->modsources[ms_modwheel];
   modsources[ms_aftertouch] = new modulation_source();
   modsources[ms_aftertouch]->output = state.voiceChannelState->pressure;
   modsources[ms_timbre] = new modulation_source();
   modsources[ms_timbre]->output = state.voiceChannelState->timbre;
	modsources[ms_pitchbend] = oscene->modsources[ms_pitchbend];	
	for(int i=0; i<n_customcontrollers; i++) modsources[ms_ctrl1+i] = oscene->modsources[ms_ctrl1+i];	
	...
	for(int i=0; i<6; i++) lfo[i].attack();	
}
...
void sub3_voice::legato(int key, int velocity, char detune)
{		
   if (state.portaphase>1)
      state.portasrc_key = state.getPitch();
	else
	{
		state.portasrc_key = ((1-state.portaphase)*state.portasrc_key + state.portaphase * state.getPitch());
	}
...
void sub3_voice::switch_toggled()
{
	update_portamento();	
	float pb = modsources[ms_pitchbend]->output;
	if(pb>0)	pb *= (float)scene->pbrange_up.val.i;	
	else		pb *= (float)scene->pbrange_dn.val.i;	
	...
	for(int i=0; i<n_oscs; i++)
	{
		if(osctype[i] != scene->osc[i].type.val.i)
		{
			...
			if (osc[i]) osc[i]->init(state.pitch);	
	...
	case fm_off:
		break;
	case fm_2to1:
		if (osc[1] && osc[0]) osc[0]->assign_fm(osc[1]->output);	
		break;
	case fm_3to2to1:
		if (osc[1] && osc[0]) osc[0]->assign_fm(osc[1]->output);	
		if (osc[2] && osc[1]) osc[1]->assign_fm(osc[2]->output);
		break;
	case fm_2and3to1:
		if (osc[0]) osc[0]->assign_fm(fmbuffer);		
		break;
	}	
	
	bool solo = (scene->solo_o1.val.b || scene->solo_o2.val.b || scene->solo_o3.val.b || scene->solo_noise.val.b || scene->solo_ring_12.val.b || scene->solo_ring_23.val.b);	
	if(solo)
	{
		if(scene->solo_o1.val.b) set_path(true,false,false,false,false,false,false);
		else if(scene->solo_o2.val.b) set_path(false,true,false,FM,false,false,false);
		else if(scene->solo_o3.val.b) set_path(false,false,true,false,false,false,false);
		else if(scene->solo_noise.val.b) set_path(false,false,false,false,false,false,true);
		else if(scene->solo_ring_12.val.b) set_path(false,false,false,false,true,false,false);
		else if(scene->solo_ring_23.val.b) set_path(false,false,false,false,false,true,false);		
	}
	else 
	{
	...
	for(int u=0; u<2; u++)
	{
		if((scene->filterunit[u].type.val.i != FBP.FU[u].type)||(scene->filterunit[u].subtype.val.i != FBP.FU[u].subtype))
		{
...
void sub3_voice::release()
{		
	modsources[ms_ampeg]->release();
	modsources[ms_filtereg]->release();

	for(int i=0; i<6; i++) lfo[i].release();

	state.gate = false;
}

void sub3_voice::uber_release()
{
	((envelope_adsr*)modsources[ms_ampeg])->uber_release();	
	state.gate = false;
	state.uberrelease = true;
}

void sub3_voice::update_portamento()
{
	//float ratemult=1;
	//if(zone->portamento_mode) ratemult = 12/(0.00001+fabs(key-portasrc_key));	
	state.portaphase += envelope_rate_linear(localcopy[scene->portamento.param_id_in_scene].f) * (scene->portamento.temposync?storage->temposyncratio:1.f);
	//powf(2,-zone->portamento)	
	if(state.portaphase<1)
	{
		state.pkey = (1.f-state.portaphase)*state.portasrc_key + (float)state.portaphase * state.getPitch();
	}
	else 
		state.pkey = state.getPitch();
}

int sub3_voice::routefilter(int r)
{
...
template<bool first> void sub3_voice::calc_ctrldata(fbq_store *Q,int e)
{	
	...
	for(int i=1; i<6; i++) 
	{
		if(scene->modsource_doprocess[ms_lfo1 + i]) lfo[i].process_block();
	}

	if(lfo[0].retrigger_EG) 
	{
		((envelope_adsr*)modsources[ms_ampeg])->retrigger();
		((envelope_adsr*)modsources[ms_filtereg])->retrigger();
	}
   ...
   modsources[ms_timbre]->output = state.voiceChannelState->timbre;
	...
	if (((envelope_adsr*)modsources[ms_ampeg])->is_idle()) state.keep_playing = false;
	...
	{		
		...
		if(modsources[src_id]) localcopy[dst_id].f += depth * modsources[src_id]->output;
		iter++;
	}		
	...
	state.pitch = state.pkey + pb + 
		localcopy[pitch_id].f * (scene->pitch.extend_range?12.f:1.f) + (12.0f * localcopy[octave_id].i);
	modsources[ms_keytrack]->output = (state.pitch - (float)scene->keytrack_root.val.i)*(1.0f/12.0f);	
	
	if(scene->modsource_doprocess[ms_polyaftertouch])
	{
		((modulation_controller*)modsources[ms_polyaftertouch])->set_target(storage->poly_aftertouch[state.scene_id&1][state.key&127]);
		((modulation_controller*)modsources[ms_polyaftertouch])->process_block();
	}
   ...
   float pan1 = limit_range(localcopy[pan_id].f + state.voiceChannelState->pan + state.mainChannelState->pan, -1.f, 1.f);
	float amp = 0.5f*amp_to_linear(localcopy[volume_id].f);	// the *0.5 multiplication will be eliminated by the 2x gain of the halfband filter	
	
	// volymkorrigering (fb_stereo uppdaterad sedan v1.2.2
	if(scene->filterblock_configuration.val.i == fb_wide) amp *= 0.6666666f;
	else if(scene->filterblock_configuration.val.i == fb_stereo) amp *= 1.3333333f;
	
	if((scene->filterblock_configuration.val.i == fb_stereo)||(scene->filterblock_configuration.val.i == fb_wide))
	{		
...
bool sub3_voice::process_block(fbq_store &Q, int Qe)
{
	...
	{
		osc[2]->process_block(
			(scene->osc[2].keytrack.val.b?state.pitch:ktrkroot) + localcopy[scene->osc[2].pitch.param_id_in_scene].f * (scene->osc[2].pitch.extend_range?12.f:1.f) + 12*scene->osc[2].octave.val.i,
			drift,
			is_wide);
		...
		{			
			...
			if(route[2]<2) accumulate_block(tblock,output[0],block_size_os_quad);
			if(route[2]>0) accumulate_block(tblockR,output[1],block_size_os_quad);
		}
	}
	...
	if(osc2 || ring12 || ring23 || (FMmode && osc1))
	{
		...
		{
			osc[1]->process_block(
				(scene->osc[1].keytrack.val.b?state.pitch:ktrkroot) + localcopy[scene->osc[1].pitch.param_id_in_scene].f * (scene->osc[1].pitch.extend_range?12.f:1.f) + 12*scene->osc[1].octave.val.i,		
				drift,
				is_wide,
				true,
		...
		{			
			...
			if(route[1]<2) accumulate_block(tblock,output[0],block_size_os_quad);
			if(route[1]>0) accumulate_block(tblockR,output[1],block_size_os_quad);
		}
	}
	...
	if(osc1 || ring12) 
	{
		if (FMmode == fm_2and3to1)
		{
			...
			osc[0]->process_block(
				(scene->osc[0].keytrack.val.b?state.pitch:ktrkroot) + localcopy[scene->osc[0].pitch.param_id_in_scene].f * (scene->osc[0].pitch.extend_range?12.f:1.f) + 12*scene->osc[0].octave.val.i,
				drift,
				is_wide, 
				true, 
				db_to_linear(localcopy[scene->fm_depth.param_id_in_scene].f));
		...
		{
			osc[0]->process_block(
				(scene->osc[0].keytrack.val.b?state.pitch:60) + localcopy[scene->osc[0].pitch.param_id_in_scene].f * (scene->osc[0].pitch.extend_range?12.f:1.f) + 12*scene->osc[0].octave.val.i,
				drift,
				is_wide,
				true,
				db_to_linear(localcopy[scene->fm_depth.param_id_in_scene].f));
		...
		{			
			...
			if(route[0]<2) accumulate_block(tblock,output[0],block_size_os_quad);
			if(route[0]>0) accumulate_block(tblockR,output[1],block_size_os_quad);
		}
	}
	...
	{ 		
		...
		if(route[3]<2) accumulate_block(tblock,output[0],block_size_os_quad);
		if(route[3]>0) accumulate_block(tblockR,output[1],block_size_os_quad);
	}
	...
	{ 
		...
		if(route[4]<2) accumulate_block(tblock,output[0],block_size_os_quad);
		if(route[4]>0) accumulate_block(tblockR,output[1],block_size_os_quad);
	}
	...
	{	
		...
		if(route[5]<2) accumulate_block(tblock,output[0],block_size_os_quad);
		if(route[5]>0) accumulate_block(tblockR,output[1],block_size_os_quad);		
	}
	...
	age++;
	if(!state.gate) age_release++;

	return state.keep_playing;
}

void sub3_voice::set_path(bool osc1, bool osc2, bool osc3, int FMmode, bool ring12, bool ring23, bool noise)
{
	this->osc1 = osc1;
	this->osc2 = osc2;
	this->osc3 = osc3;
...
void sub3_voice::SetQFB(fbq_store *Q, int e)		// Q == 0 betyder init
{		
	fbq = Q;
	fbqi = e;
	...
	float Gain = db_to_linear(localcopy[id_vca].f + localcopy[id_vcavel].f*(1.f-state.fvel)) * modsources[ms_ampeg]->output;		
	...
	if(Q)
	{
		...
		float cutoffA = localcopy[id_cfa].f + localcopy[id_kta].f*keytrack + localcopy[id_emoda].f*fenv;
		float cutoffB = localcopy[id_cfb].f + localcopy[id_ktb].f*keytrack + localcopy[id_emodb].f*fenv;

		if(scene->f2_cutoff_is_offset.val.b) cutoffB += cutoffA;

		CM[0].MakeCoeffs(cutoffA, localcopy[id_resoa].f, scene->filterunit[0].type.val.i,scene->filterunit[0].subtype.val.i);
		CM[1].MakeCoeffs(cutoffB, 
			scene->f2_link_resonance.val.b ? localcopy[id_resoa].f : localcopy[id_resob].f, 
		...
		{	
			...
			{												
				...
				if(scene->filterunit[u].type.val.i == fut_lpmoog)
					Q->FU[u].WP[0] = scene->filterunit[u].subtype.val.i;	// lpmoog's output stage finns i WP[0] för hela quaden
