...
sub3_voice::sub3_voice(sub3_storage* storage,
                       sub3_scene* oscene,
                       pdata* params,
                       int key,
                       int velocity,
...
//: fb(storage,oscene)
{
   ...
   ((modulation_controller*)modsources[ms_polyaftertouch])
...
template <bool first> void sub3_voice::calc_ctrldata(fbq_store* Q, int e)
{
   ...
   if (((envelope_adsr*)modsources[ms_ampeg])->is_idle())
