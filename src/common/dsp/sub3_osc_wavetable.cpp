...
osc_wavetable::osc_wavetable(sub3_storage *storage, sub3_osc *oscdata, pdata *localcopy) : oscillator_BLIT(storage,oscdata,localcopy)
{	
...
void osc_wavetable::init(float pitch, bool is_display)
{
	...
	for(i=0; i<n_unison; i++){						
		{			
			float s = 0.f; 
			oscstate[i] = 0; 
			if (oscdata->retrigger.val.b) s = 0.f; //(oscdata->startphase.val.f) * (float)oscdata->wt.size;
			else if(!is_display)				
			{
...
{		
	...
	if(state[voice] >= (wtsize-1)) dt += (1-formant);
	t = dt * tempt;		
	...
	float newlevel = distort_level( oscdata->wt.TableF32WeakPointers[mipmap[voice]][tableid][state[voice]]*(1.f-tblip_ipol) + oscdata->wt.TableF32WeakPointers[mipmap[voice]][tableid+1][state[voice]]*tblip_ipol );
...
void osc_wavetable::update_lagvals()
{		
	...
	float hpf2 = min(integrator_hpf,powf(hpf_cycle_loss,4*invt));	// TODO !!! ACHTUNG! gör lookup-table
...
{			
	...
	if((oscdata->wt.n_tables == 1) || (tableid >= oscdata->wt.n_tables))  // tableid-range kan ha ändrats under tiden, gör koll
	{
		tableipol = 0.f;
		tableid = 0;
		last_tableid = 0;
	...
	if(FM)
	{			
		/*FMfilter.coeff_HP(FMfilter.calc_omega(pitch_t - 24.f - 48.f * oscdata->p[id_detune].val.f),0.707);
		FMfilter.process_block(&master_osc[0]);
	...
	else
	{
		...
		{	
			...
			while(oscstate[l] < a) convolute(l,false,stereo);
			oscstate[l] -= a;			
		}		
		//li_DC.set_target(dc);	
	}	
