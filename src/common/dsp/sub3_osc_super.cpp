...
oscillator_BLIT::oscillator_BLIT(sub3_storage *storage, sub3_osc *oscdata, pdata *localcopy) : oscillator(storage,oscdata,localcopy)
{		
	integrator_hpf = (1.f - 2.f*20.f*samplerate_inv);
	integrator_hpf *= integrator_hpf;
}

void oscillator_BLIT::prepare_unison(int voices)
{
...
osc_super::osc_super(sub3_storage *storage, sub3_osc *oscdata, pdata *localcopy) : oscillator_BLIT(storage,oscdata,localcopy)
{		
}
...
void osc_super::init(float pitch, bool is_display)
{
	assert(storage);
	first_run = true;				
	...
	{				
		...
		{			
			...
			//double t = drand * max(2.0,dsamplerate_os / (16.35159783 * pow((double)1.05946309435,(double)pitch)));	
...
void osc_super::init_ctrltypes()
{
	oscdata->p[0].set_name("Shape");		oscdata->p[0].set_type(ct_percent_bidirectional);
	oscdata->p[1].set_name("Width");		oscdata->p[1].set_type(ct_percent);	oscdata->p[1].val_default.f = 0.5f;
	oscdata->p[2].set_name("Sub Width");	oscdata->p[2].set_type(ct_percent); oscdata->p[2].val_default.f = 0.5f;
	oscdata->p[3].set_name("Sub Level");	oscdata->p[3].set_type(ct_percent);
	oscdata->p[4].set_name("Sync");			oscdata->p[4].set_type(ct_syncpitch);
	oscdata->p[5].set_name("Uni Spread");	oscdata->p[5].set_type(ct_oscspread);	
	oscdata->p[6].set_name("Uni Count");	oscdata->p[6].set_type(ct_osccount);	
}
void osc_super::init_default_values()
{
...
template<bool FM> void osc_super::convolute(int voice, bool stereo)
{
	const bool NODC = false;
	//assert(oscstate[voice]>=0.f);
	float detune = drift*driftlfo[voice];
	if(n_unison>1) detune += localcopy[id_detune].f*(detune_bias*(float)voice + detune_offset);
	float wf = l_shape.v;
	float sub = l_sub.v;	
	...
	{		
		if(FM) ipos = (unsigned int)(p24*(syncstate[voice]*pitchmult_inv*FMmul_inv));	
		else ipos = (unsigned int)(p24*(syncstate[voice]*pitchmult_inv));	
		//double t = max(0.5,dsamplerate_os * (1/8.175798915) * note_to_pitch_inv(pitch + detune) * 2);
	...
	{
		if(FM) ipos = (unsigned int)(p24*(oscstate[voice]*pitchmult_inv*FMmul_inv));	
		else ipos = (unsigned int)(p24*(oscstate[voice]*pitchmult_inv));	
	}	
	
	unsigned int delay;
	if(FM) delay = FMdelay;
	else delay = ((ipos>>24)&0x3f);	
	...
	case 0:
		{
			...
			last_level[voice] = tg;		
			if(!NODC) last_level[voice] -= (pwidth[voice])*(pwidth2[voice])*(1.f+wf)*(1.f-sub);		// calculate the level the sub-cycle will have at the end of it's duration taking DC into account
			break;
		}
	case 1:
		g = wf*(1.f-sub) - sub;	
		last_level[voice] += g;
		if(!NODC) last_level[voice] -= (1-pwidth[voice])*(2-pwidth2[voice])*(1+wf)*(1.f-sub);
		break;
	case 2:		
		g = 1.f-sub;
		last_level[voice] += g;
		if(!NODC) last_level[voice] -= (pwidth[voice])*(2-pwidth2[voice])*(1+wf)*(1.f-sub);
		break;
	case 3:
		g = wf*(1.f-sub) + sub;
		last_level[voice] += g;		
		if(!NODC) last_level[voice] -= (1-pwidth[voice])*(pwidth2[voice])*(1+wf)*(1.f-sub);
		break;
	};		
...
*/
	...
	if(stereo)
	{
		...
		vector unsigned char mask, maskstore;	// since both buffers are aligned and read the same position, the same mask can be used
...
{		
	...
	float invt = 4.f * min(1.0,(8.175798915 * note_to_pitch(pitch + l_sync.v)) * dsamplerate_os_inv);		
...
void osc_super::process_block(float pitch0, float drift, bool stereo, bool FM, float depth)
{			
	this->pitch = min(148.f,pitch0);
	this->drift = drift;			
	pitchmult_inv = Max(1.0,dsamplerate_os * (1.f / 8.175798915f) * note_to_pitch_inv(pitch));	

	pitchmult = 1.f / pitchmult_inv; // denna måste vara en riktig division, reciprocal-approx är inte precis nog
	...
	{				
		...
		{			
			...
			{					
				...
				{		
					FMmul_inv = rcp(fmmul);
					// divisionen racar med ökningen av oscstate så att den aldrig kommer ur loopen	detta blir unsafe
					// fucka inte med oscstate utan gör divisionen inne i convolute istället					
					convolute<true>(l,stereo);
				}
				
				oscstate[l] -= a;
				if (l_sync.v>0)  syncstate[l] -= a;			
			}			
		}		
	}
	else
	...
	{			
		...
		osc_out2 = _mm_add_ss(
			_mm_mul_ss(osc_out2,char_a1),
			_mm_add_ss(
				_mm_mul_ss(osc_out,char_b0),
				_mm_mul_ss(LastOscOut,char_b1)));
		...
		if(stereo)
		{
			...
			osc_out2R = _mm_add_ss(
				...
				_mm_add_ss(
					_mm_mul_ss(osc_outR,char_b0),
					_mm_mul_ss(LastOscOutR,char_b1)));
	...
	if(stereo) clear_block(&oscbufferR[bufpos],block_size_os_quad);
