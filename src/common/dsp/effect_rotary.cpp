...
rotary_speaker::rotary_speaker(sub3_storage *storage, sub3_fx *fxdata, pdata* pd) 
: baseeffect(storage,fxdata,pd), xover(storage), lowbass(storage)
{
...
void rotary_speaker::init_default_values()
{
...
const char* rotary_speaker::group_label(int id)
{
	switch (id)
	{
	case 0:
...
int rotary_speaker::group_label_ypos(int id)
{
	switch (id)
	{
	case 0:
...
void rotary_speaker::init_ctrltypes()
{
	baseeffect::init_ctrltypes();

	fxdata->p[0].set_name("Horn Rate");			fxdata->p[0].set_type(ct_lforate);
	fxdata->p[1].set_name("Doppler");			fxdata->p[1].set_type(ct_percent);
	fxdata->p[2].set_name("Amp Mod");			fxdata->p[2].set_type(ct_percent);			
...
void rotary_speaker::process_only_control()
{
...
void rotary_speaker::process(float *dataL, float* dataR)
{	
	lfo.set_rate(2*M_PI*powf(2,*f[rsp_rate])*dsamplerate_inv*block_size);
	...
	{	
		...
		int sincL = FIRipol_N*limit_range((int)(FIRipol_M*(float(i_dtimeL + 1)-dL.v)),0,FIRipol_M-1);
		int sincR = FIRipol_N*limit_range((int)(FIRipol_M*(float(i_dtimeR + 1)-dR.v)),0,FIRipol_M-1);		
		...
		{
			tbufferL[k] += buffer[(rpL-i)&(max_delay_length-1)]*sinctable1X[sincL + FIRipol_N - i];
			tbufferR[k] += buffer[(rpR-i)&(max_delay_length-1)]*sinctable1X[sincR + FIRipol_N - i];			
		}	
		dL.process();
		dR.process();
	}
