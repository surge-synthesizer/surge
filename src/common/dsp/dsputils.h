...
public:
	...
	void reset()
	{
		if (first_run_checks) first_run = true; 
		new_v = 0;
		v = 0;
		dv = 0;
...
inline void trixpan(float &L, float &R, float x)		// panning that always lets both channels through unattenuated (seperate hard-panning)
{
	if(x<0.f)
	{
		L = L - x*R;
...
/*#else
	...
	__m128 denom = _mm_add_ss(_mm_add_ss(one,x), _mm_add_ss(xx,_mm_mul_ss(xx,_mm_mul_ss(_mm_load_ss(&a),x)));
...
// bollocks to it
inline float amp_to_linear(float x){ x = max(0.f, x); return x*x*x; }
inline float linear_to_amp(float x){ return powf(limit_range(x,0.0000000001f,1.f),1.f/3.f); }
inline float amp_to_db(float x){ return limit_range((float)(6.f*3.f*log(x)/log(2.f)), -192.f, 96.f); }
inline float db_to_amp(float x){ return limit_range(powf((10.f/3.f),0.05f*x),0.f,1.f); }
...
inline double blackman_harris(int i, int n){
	//if (i>=n) return 0;
	return (0.35875 - 0.48829*cos(2*M_PI*i/(n-1)) + 0.14128*cos(4*M_PI*i/(n-1)) - 0.01168*cos(6*M_PI*i/(n-1)));
}
...
inline double symmetric_blackman_harris(double i, int n){
	...
	i -= (n/2);
	//return (0.42 - 0.5*cos(2*M_PI*i/(n)) + 0.08*cos(4*M_PI*i/(n)));
	return (0.35875 - 0.48829*cos(2*M_PI*i/(n)) + 0.14128*cos(4*M_PI*i/(n-1)) - 0.01168*cos(6*M_PI*i/(n)));
}
...
inline bool is_yes(const char * str)
{
	if (!str) return 0;
#if WINDOWS
	if (_stricmp(str,"yes") == 0) return 1;
#else
