...
freqshift::freqshift(sub3_storage *storage, sub3_fx *fxdata, pdata* pd) 
: baseeffect(storage,fxdata,pd), time(0.0001), shiftL(0.01), shiftR(0.01), //fiL(true), frL(true), fiR(true), frR(true)
fr(6,true),fi(6,true)
{
...
void freqshift::setvars(bool init)
{
	feedback.newValue(amp_to_linear(*f[fsp_feedback]));		
			
	if(init) time.newValue((fxdata->p[fsp_delay].temposync?storage->temposyncratio_inv:1.f) * samplerate * note_to_pitch(12* fxdata->p[fsp_delay].val.f) - FIRoffset);	
	else time.newValue((fxdata->p[fsp_delay].temposync?storage->temposyncratio_inv:1.f) * samplerate * note_to_pitch(12* *f[fsp_delay]) - FIRoffset);	
...
void freqshift::process(float *dataL, float *dataR)
{	
	setvars(false);
	
	int k;	
	_MM_ALIGN16 float L[block_size],R[block_size],Li[block_size],Ri[block_size],Lr[block_size],Rr[block_size];
	...
	for(k=0; k<block_size; k++)
	{
		...
		int sinc = FIRipol_N*limit_range((int)(FIRipol_M*(float(i_dtime + 1)-time.v)),0,FIRipol_M-1);	
...
void freqshift::suspend()
{ 
	init();
	ringout = 10000000;
}

const char* freqshift::group_label(int id)
{
	switch (id)
	{
	case 0:
...
int freqshift::group_label_ypos(int id)
{
	switch (id)
	{
	case 0:
...
void freqshift::init_ctrltypes()
{
	baseeffect::init_ctrltypes();

	fxdata->p[fsp_shift].set_name("Left");				fxdata->p[fsp_shift].set_type(ct_freq_shift);
	fxdata->p[fsp_rmult].set_name("Right");				fxdata->p[fsp_rmult].set_type(ct_percent_bidirectional);
	fxdata->p[fsp_delay].set_name("Delay");				fxdata->p[fsp_delay].set_type(ct_envtime);
	fxdata->p[fsp_feedback].set_name("Feedback");		fxdata->p[fsp_feedback].set_type(ct_amplitude);	
	fxdata->p[fsp_mix].set_name("Mix");					fxdata->p[fsp_mix].set_type(ct_percent);	
...
void freqshift::init_default_values()
{
