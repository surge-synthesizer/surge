...
template<int config,bool A,bool WS,bool B> void ProcessFBQuad(fbq_store &d, fbq_global &g, float *OutL, float *OutR)
{		
	const __m128 hb_c = _mm_set1_ps(0.5f);	// om denna ‰ndras frÂn 0.5, se till att ‰ndra i koden eftersom det antas att den ‰r h‰lften
	...
	case fb_serial2:
		...
		break;
	case fb_serial3:	// filter 2 is only heard in the feedback path, good for physical modelling with comb as f2
		for(int k=0; k<block_size_os; k++)
		{							
			d.FB = _mm_add_ps(d.FB,d.dFB);
			__m128 input = vMul(d.FB, d.FBlineL);
...
template<int config> FBQFPtr GetFBQPointer2(bool A,bool WS,bool B)
{
	if(A)
	{
		if(B)
		{
			if(WS) return ProcessFBQuad<config,1,1,1>;
			else return ProcessFBQuad<config,1,0,1>;
		}
		else
		{
			if(WS) return ProcessFBQuad<config,1,1,0>;
			else return ProcessFBQuad<config,1,0,0>;
		}
	}
	else
	{
			if(B)
		{
			if(WS) return ProcessFBQuad<config,0,1,1>;
			else return ProcessFBQuad<config,0,0,1>;
		}
		else
		{
			if(WS) return ProcessFBQuad<config,0,1,0>;
			else return ProcessFBQuad<config,0,0,0>;
		}
	}
	return 0;
}
