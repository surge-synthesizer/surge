...
#if PPC
vFloat SVFLP12Aquad( fuq_store * __restrict f, __m128 in )
{
...
#else
__m128 SVFLP12Aquad( fuq_store * __restrict f, __m128 in )
{
...
#endif
#if PPC
vFloat SVFLP24Aquad( fuq_store * __restrict f, __m128 in )
{
	f->C[0] = vec_add(f->C[0], f->dC[0]);	// F1
	f->C[1] = vec_add(f->C[1], f->dC[1]);	// Q1
...
#else
__m128 SVFLP24Aquad( fuq_store * __restrict f, __m128 in )
{
	f->C[0] = _mm_add_ps(f->C[0],f->dC[0]);	// F1
	f->C[1] = _mm_add_ps(f->C[1],f->dC[1]);	// Q1
...
#endif
#if PPC
vFloat SVFHP24Aquad( fuq_store * __restrict f, __m128 in )
{
	f->C[0] = vec_add(f->C[0],f->dC[0]);	// F1
	f->C[1] = vec_add(f->C[1],f->dC[1]);	// Q1
...
#else
__m128 SVFHP24Aquad( fuq_store * __restrict f, __m128 in )
{
	f->C[0] = _mm_add_ps(f->C[0], f->dC[0]);	// F1
	f->C[1] = _mm_add_ps(f->C[1], f->dC[1]);	// Q1
...
#if PPC
vFloat SVFBP24Aquad( fuq_store * __restrict f, __m128 in )
{
	f->C[0] = vec_add(f->C[0], f->dC[0]);	// F1
	f->C[1] = vec_add(f->C[1], f->dC[1]);	// Q1
...
#else
__m128 SVFBP24Aquad( fuq_store * __restrict f, __m128 in )
{
	f->C[0] = _mm_add_ps(f->C[0],f->dC[0]);	// F1
	f->C[1] = _mm_add_ps(f->C[1],f->dC[1]);	// Q1
...
#if PPC
vFloat SVFHP12Aquad( fuq_store * __restrict f, __m128 in )
{
	f->C[0] = vec_add(f->C[0],f->dC[0]);	// F1
	f->C[1] = vec_add(f->C[1],f->dC[1]);	// Q1
...
#else
__m128 SVFHP12Aquad( fuq_store * __restrict f, __m128 in )
{
f->C[0] = _mm_add_ps(f->C[0],f->dC[0]);	// F1
	f->C[1] = _mm_add_ps(f->C[1],f->dC[1]);	// Q1
...
#endif
#if PPC
vFloat SVFBP12Aquad( fuq_store * __restrict f, __m128 in )
{
	f->C[0] = vec_add(f->C[0],f->dC[0]);	// F1
	f->C[1] = vec_add(f->C[1],f->dC[1]);	// Q1
...
#else
__m128 SVFBP12Aquad( fuq_store * __restrict f, __m128 in )
{
f->C[0] = _mm_add_ps(f->C[0],f->dC[0]);	// F1
	f->C[1] = _mm_add_ps(f->C[1],f->dC[1]);	// Q1
...
#if PPC
#else
__m128 IIR12Aquad( fuq_store * __restrict f, __m128 in )
{
	...
	__m128 y = _mm_add_ps(_mm_add_ps(_mm_mul_ps(f->C[6],g2),_mm_mul_ps(f->C[5],g1)), _mm_mul_ps(f->C[4],f1));
...
#endif
#if PPC
vFloat IIR12Bquad( fuq_store * __restrict f, vFloat in )
{									
...
#else
__m128 IIR12Bquad( fuq_store * __restrict f, __m128 in )
{									
	__m128 f2 = _mm_sub_ps(_mm_mul_ps(f->C[3],in), _mm_mul_ps(f->C[1],f->R[1]));		// Q2*in - K2*R1
	f->C[1] = _mm_add_ps(f->C[1],f->dC[1]);	// K2
	f->C[3] = _mm_add_ps(f->C[3],f->dC[3]);	// Q2
...
#if PPC
#else
__m128 IIR12WDFquad( fuq_store * __restrict f, __m128 in )
{
	...
	__m128 y = _mm_add_ps(_mm_add_ps(_mm_mul_ps(f->C[4],f->R[0]),_mm_mul_ps(f->C[6],in)),_mm_mul_ps(f->C[5],f->R[1]));
	__m128 t = _mm_add_ps(in,_mm_add_ps(_mm_mul_ps(f->C[2],f->R[0]),_mm_mul_ps(f->C[3],f->R[1])));
...
#endif
#if PPC
__m128 IIR12CFCquad( fuq_store * __restrict f, __m128 in )
{
...
__m128 IIR12CFCquad( fuq_store * __restrict f, __m128 in )
{
	...
	__m128 y = _mm_add_ps(_mm_add_ps(_mm_mul_ps(f->C[4],f->R[0]),_mm_mul_ps(f->C[6],in)),_mm_mul_ps(f->C[5],f->R[1]));
	__m128 s1 = _mm_add_ps(_mm_mul_ps(in,f->C[2]),_mm_sub_ps(_mm_mul_ps(f->C[0],f->R[0]),_mm_mul_ps(f->C[1],f->R[1])));
...
#if PPC
#else
__m128 IIR12CFLquad( fuq_store * __restrict f, __m128 in )	
{
	...
	__m128 s1 = _mm_add_ps(_mm_mul_ps(in,f->C[2]),_mm_sub_ps(_mm_mul_ps(ar,f->R[0]),_mm_mul_ps(ai,f->R[1])));
...
#endif
#if PPC
__m128 IIR24CFCquad( fuq_store * __restrict f, __m128 in )
{
	// state-space med clipgain (2nd order, direkt registerbegr‰nsning)
...
__m128 IIR24CFCquad( fuq_store * __restrict f, __m128 in )
{
	...
	__m128 y2 = _mm_add_ps(_mm_add_ps(_mm_mul_ps(f->C[4],f->R[3]),_mm_mul_ps(f->C[6],y)),_mm_mul_ps(f->C[5],f->R[4]));
	__m128 s3 = _mm_add_ps(_mm_mul_ps(y,f->C[2]),_mm_sub_ps(_mm_mul_ps(f->C[0],f->R[3]),_mm_mul_ps(f->C[1],f->R[4])));
...
#if PPC
#else
__m128 IIR24CFLquad( fuq_store * __restrict f, __m128 in )	
{
	// State-space med mjukare limiter
	...
	__m128 y2 = _mm_add_ps(_mm_add_ps(_mm_mul_ps(f->C[4],f->R[3]),_mm_mul_ps(f->C[6],y)),_mm_mul_ps(f->C[5],f->R[4]));	
	__m128 s3 = _mm_add_ps(_mm_mul_ps(y,f->C[2]),_mm_sub_ps(_mm_mul_ps(ar,f->R[3]),_mm_mul_ps(ai,f->R[4])));
...
#endif
#if PPC
__m128 IIR24Bquad( fuq_store * __restrict f, __m128 in )
{
	f->C[1] = vec_add(f->C[1],f->dC[1]);	// K2
	f->C[3] = vec_add(f->C[3],f->dC[3]);	// Q2								
	f->C[0] = vec_add(f->C[0],f->dC[0]);	// K1
...
__m128 IIR24Bquad( fuq_store * __restrict f, __m128 in )
{
	...
	__m128 y1 = _mm_add_ps(_mm_add_ps(_mm_mul_ps(f->C[6],g2),_mm_mul_ps(f->C[5],g1)), _mm_mul_ps(f->C[4],f1));	
	...
	__m128 y2 = _mm_add_ps(_mm_add_ps(_mm_mul_ps(f->C[6],g2),_mm_mul_ps(f->C[5],g1)), _mm_mul_ps(f->C[4],f1));	
...
#endif
#if PPC
__m128 LPMOOGquad( fuq_store * __restrict f, __m128 in )
{	
	...
	f->R[0] = vec_softclip8(vec_madd(f->C[1], 
							vec_sub(vec_sub(vec_madd(in, f->C[0], zero), vec_madd( f->C[2],vec_add(f->R[3],f->R[4]), zero)), f->R[0])
							, f->R[0]));			
...
{	
	...
	f->R[0] = softclip8_ps(_mm_add_ps(f->R[0], _mm_mul_ps(f->C[1], 
		_mm_sub_ps(_mm_sub_ps(_mm_mul_ps(in, f->C[0]), _mm_mul_ps( f->C[2],_mm_add_ps(f->R[3],f->R[4]))), f->R[0])
		)));			
...
#endif
#if PPC
__m128 SNHquad( fuq_store * __restrict f, __m128 in )
{	
	f->C[0] = vec_add(f->C[0],f->dC[0]);	
	f->C[1] = vec_add(f->C[1],f->dC[1]);			
...
{	
	...
	f->R[1] = _mm_or_ps(_mm_andnot_ps(mask,f->R[1]), _mm_and_ps(mask,softclip_ps(_mm_sub_ps(in,_mm_mul_ps(f->C[1],f->R[1])))));
...
#endif
#if PPC
__m128 COMBquad_PPC( fuq_store * __restrict f, __m128 in )
{	
	...
	for(int i=0; i<4; i++)
	{
		...
		{					
			...
			float t = *((float*)&d + i);
			f->DB[i][f->WP[i]] = t;
			if(f->WP[i]<FIRipol_N) f->DB[i][f->WP[i]+max_fb_comb] = t;
...
#else
#if !_M_X64
__m128 COMBquad_SSE1( fuq_store * __restrict f, __m128 in )
{	
	...
	for(int i=0; i<4; i++)
	{
		...
		{					
			...
			if(f->WP[i]<FIRipol_N) _mm_store_ss(&f->DB[i][f->WP[i]+max_fb_comb], t);
...
__m128 COMBquad_SSE2( fuq_store * __restrict f, __m128 in )
{	
	assert(FIRipol_M == 256);	// changing the constant requires updating the code below
	const __m128 m256 = _mm_set1_ps(256.f);		
	const __m128i m0xff = _mm_set1_epi32(0xff);
...
__m128 DIGI_SSE2(__m128 in, __m128 drive)
{
   // v1.2: return (double)((int)((double)(x*p0inv*16.f+1.0)))*p0*0.0625f;	
	...
	return _mm_mul_ps(
		...
		_mm_mul_ps(
		m16inv, 
		_mm_sub_ps(_mm_cvtepi32_ps(a), mofs)));
}
...
{		
	// TODO byta denna n‰r optimerad sine() ‰r redo
...
#else
#if !_M_X64
		if(SSE_VERSION<2) return COMBquad_SSE1;
#endif
...
{	
	...
	case wst_digi:				
#if !_M_X64
		if(SSE_VERSION<2) return DIGI_SSE1;
#endif
		return DIGI_SSE2;
	case wst_hard:
		return CLIP;
	case wst_sinus:
#if !_M_X64
		if(SSE_VERSION<2) return SINUS_SSE1;
#endif
		return SINUS_SSE2;
	case wst_asym:
#if !_M_X64
		if(SSE_VERSION<2) return ASYM_SSE1;
#endif
		return ASYM_SSE2;
	case wst_tanh:
		return TANH;
