...
biquadunit::biquadunit(sub3_storage* storage)
{
...
void biquadunit::coeff_LP2B(double omega, double Q)
{
   ...
   else
   {
      ...
      double den = (w_sq * w_sq) + (M_PI * M_PI * M_PI * M_PI) + w_sq * (M_PI * M_PI) * (1 / Q - 2);
...
void biquadunit::coeff_BP(double omega, double Q)
{
...
void biquadunit::coeff_NOTCH(double omega, double QQ)
{
   if (omega > M_PI)
      set_coef(1, 0, 0, 1, 0, 0);
   else
   {
      double cosi = cos(omega), sinu = sin(omega), reso = limit_range(QQ, 0.0, 1.0),
             q = 1 / (0.02 + 30 * reso * reso), alpha = sinu / (2 * q), b0 = 1.0, b1 = -2.0 * cosi,
             b2 = 1.0, a0 = 1.0 + alpha, a1 = -2.0 * cosi, a2 = 1.0 - alpha;
...
void biquadunit::process_block_to(float* __restrict data, float* __restrict dataout)
{
   /*if(storage->SSE2) process_block_SSE2(data);
   else*/
   {
...
void biquadunit::process_block_slowlag(float* __restrict dataL, float* __restrict dataR)
{
