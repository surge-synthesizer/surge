...
public:
	modulation_lfo();
	void assign(sub3_storage *storage, sub3_lfo *lfo, pdata *localcopy, sub3_voice_state *state, sub3_stepsequence *ss, bool is_display=false);
	float bend1(float x);
	float bend2(float x);
	float bend3(float x);
	virtual void attack();
	...
	virtual const char* get_title(){ return "LFO"; }
	virtual int get_type() {return mst_lfo; }
