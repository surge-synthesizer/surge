...
class envelope_adsr : public modulation_source
{
public:
	envelope_adsr(sub3_storage *storage, sub3_adsr *adsr, pdata *localcopy, sub3_voice_state *state)
	{
	...
	virtual const char* get_title(){ return "envelope"; }
	virtual int get_type() {return mst_adsr; }
	virtual bool per_voice(){ return true; }
	...
	{	
      if (lc[mode].b)
      {
         ...
         float coef_A = powf(2.f, min(0.f, coeff_offset - lc[a].f * (adsr->a.temposync?storage->temposyncratio:1.f)));
         float coef_D = powf(2.f, min(0.f, coeff_offset - lc[d].f * (adsr->d.temposync?storage->temposyncratio:1.f)));
         float coef_R = envstate == s_uberrelease 
            ? 6.f 
            : powf(2.f, min(0.f, coeff_offset - lc[r].f * (adsr->r.temposync?storage->temposyncratio:1.f)));
      ...
      else
      {
         ...
         case(s_attack):
            {							
               phase += envelope_rate_linear(lc[a].f) * (adsr->a.temposync?storage->temposyncratio:1.f);
               if(phase >= 1)
               {					
                  phase = 1;
                  envstate = s_decay;
         ...
         case(s_decay):
            {
               ...
               float rate = envelope_rate_linear(lc[d].f) * (adsr->d.temposync?storage->temposyncratio:1.f);
         ...
         case(s_release):
            {
               phase -= envelope_rate_linear(lc[r].f) * (adsr->r.temposync?storage->temposyncratio:1.f);
               output = phase;
               for(int i=0; i<lc[r_s].i; i++) output *= phase;
