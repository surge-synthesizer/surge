...
modulation_lfo::modulation_lfo()
{
}

void modulation_lfo::assign(sub3_storage *storage, sub3_lfo *lfo, pdata *localcopy, sub3_voice_state *state, sub3_stepsequence *ss, bool is_display)
{
	...
	if(is_display) srand(17);		
	noise = 0.f;
	noised1 = 0.f;
	target = 0.f;	
	for(int i=0; i<4; i++) wf_history[i] = 0.f; //((float) rand()/RAND_MAX)*2.f - 1.f;	
}

float modulation_lfo::bend1(float x)
{
...
float modulation_lfo::bend2(float x)
{
	float a = localcopy[ideform].f;							
...
float modulation_lfo::bend3(float x)
{
...
{	
	...
	else*/
	{
		...
		if(localcopy[idelay].f == lfo->delay.val_min.f)
		{
			...
			if(localcopy[iattack].f == lfo->attack.val_min.f) 
			{
				env_state = lenv_hold;
				env_val = 1.f;
				if(localcopy[ihold].f == lfo->hold.val_min.f) env_state = lenv_decay;			
			}
		}	
	}	
	...
	{				
		...
		if (lfo->shape.val.i == ls_stepseq) phase = 0.f;
		step = 0;
	}
	else
	{
		float phaseslider;
		if (lfo->shape.val.i == ls_stepseq) phaseslider = 0.f;		// använd phase som shuffle-param istället
		//else if(state) phaseslider = lfo->start_phase.val.f;
		else phaseslider = localcopy[startphase].f;
		...
		case lm_freerun:
			{
				...
				phase = (float)modf(phaseslider + 0.5f*storage->songpos*pow(2.0,(double)localcopy[rate].f),&ipart);
				int i = (int)ipart;
	...
	{
	case ls_snh:		
		noise = 0.f; noised1 = 0.f; target = 0.f;
	...
	case ls_stepseq:	
		{			
			...
			step ++; if (step>ss->loop_end) step = ss->loop_start;
...
void modulation_lfo::process_block()
{
	retrigger_EG = false;
	int s = lfo->shape.val.i;

	float frate = envelope_rate_linear(-localcopy[rate].f);
	if(lfo->rate.temposync) frate *= storage->temposyncratio;
	...
	if (env_state != lenv_stuck)
	{
		...
		case lenv_delay:
			...
			if(lfo->delay.temposync) envrate *= storage->temposyncratio;
			break;
		case lenv_attack:
			envrate = envelope_rate_linear(localcopy[iattack].f);
			if(lfo->attack.temposync) envrate *= storage->temposyncratio;
			break;
		case lenv_hold:
			envrate = envelope_rate_linear(localcopy[ihold].f);
			if(lfo->hold.temposync) envrate *= storage->temposyncratio;
			break;
		case lenv_decay:
			envrate = envelope_rate_linear(localcopy[idecay].f);
			if(lfo->decay.temposync) envrate *= storage->temposyncratio;
			break;
		case lenv_release:
			envrate = envelope_rate_linear(localcopy[irelease].f);
			if(lfo->release.temposync) envrate *= storage->temposyncratio;
			break;
		};					
		env_phase += envrate;

		float sustainlevel = localcopy[isustain].f;
		//sustainlevel = sustainlevel*(1.f + localcopy[ideform].f) - sustainlevel*sustainlevel*localcopy[ideform].f;
		//sustainlevel = sustainlevel / (sustainlevel*localcopy[ideform].f + 1 - localcopy[ideform].f);
		//float dd = (localcopy[ideform].f - 1);
		//if (s == ls_constant1) sustainlevel = 0.5f * (sqrt(4.f*sustainlevel + dd*dd) + dd);
		//if (s == ls_constant1) sustainlevel = 1.f / (1.f + localcopy[ideform].f);
		//a = (1.f-localcopy[ideform].f) + localcopy[ideform].f*env_val;	
		// u = e*a;
	...
	if(phase>1)
	{
		...
		case ls_noise:
			...
			break;	
		case ls_stepseq:
			if (ss->trigmask & (1<<step)) retrigger_EG = true; 
			step ++;
			shuffle_id = (shuffle_id+1)&1;
			if(shuffle_id) ratemult = 1.f/max(0.01f,1.f - 0.5f*lfo->start_phase.val.f);
			else ratemult = 1.f/(1.f + 0.5f*lfo->start_phase.val.f);
			
			if (step>ss->loop_end) step = ss->loop_start;
			wf_history[3] = wf_history[2];
			wf_history[2] = wf_history[1];
			wf_history[1] = wf_history[0];
			wf_history[0] = ss->steps[step&(n_stepseqsteps-1)];			
	...
	case ls_stepseq:
		//iout = wf_history[0];
		{
			...
			if(df>0.5f)
			{
				...
				float cubic = CubicInterpolate(wf_history[3],wf_history[2],wf_history[1],wf_history[0],phase);
	...
	float io2 = iout;
	if(lfo->unipolar.val.b && (s != ls_stepseq)) io2 = 0.5f + 0.5f*io2;
