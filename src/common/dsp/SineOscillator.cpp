...
void SineOscillator::process_block(float pitch, float drift, bool stereo, bool FM, float fmdepth)
{
   ...
   for (int l = 0; l < n_unison; l++)
   {
      ...
      if (n_unison > 1)
      {
         ...
         {
            detune += oscdata->p[sine_unison_detune].get_extended(localcopy[oscdata->p[sine_unison_detune].param_id_in_scene].f) *
                      storage->note_to_pitch_inv_ignoring_tuning(std::min(148.f, pitch)) * 16 / 0.9443 * (detune_bias * float(l) + detune_offset);
         }
         else
         {
            detune += oscdata->p[sine_unison_detune].get_extended(localcopy[id_detune].f) * (detune_bias * float(l) + detune_offset);
         }
      }
...
void SineOscillator::applyFilter()
{
   if (!oscdata->p[sine_lowcut].deactivated)
      hp.coeff_HP(hp.calc_omega(localcopy[oscdata->p[sine_lowcut].param_id_in_scene].f / 12.0) / OSC_OVERSAMPLING, 0.707);
   if (!oscdata->p[sine_highcut].deactivated)
      lp.coeff_LP2B(lp.calc_omega(localcopy[oscdata->p[sine_highcut].param_id_in_scene].f / 12.0) / OSC_OVERSAMPLING, 0.707);
                                              ...
                                              int currentSynthStreamingRevision)
{
   ...
   if (streamingRevision <= 12)
   {
      ...
      if (oscdata->p[sine_shape].val.i < 0 || (oscdata->p[sine_shape].val.i >= (sizeof wave_remap) / sizeof *wave_remap))
         oscdata->p[sine_shape].val.i = oscdata->p[sine_shape].val_min.i;
      else
      {
