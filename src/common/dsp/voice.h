...
public:
	...
	_MM_ALIGN16 float fmbuffer[block_size_os];		// used for the 2>1<3 FM-mode (behöver pointern tidigare)	

   sub3_voice(sub3_storage *storage, sub3_scene *scene, pdata *params, int key, int velocity, int channel, int scene_id, float detune, MidiKeyState* keyState, MidiChannelState* mainChannelState, MidiChannelState* voiceChannelState);
	~sub3_voice();
...
private:	
	...
	// filterblock stuff
	int id_cfa,id_cfb,id_kta,id_ktb,id_emoda,id_emodb,id_resoa,id_resob,id_drive,id_vca,id_vcavel,id_fbalance,id_feedback;
};