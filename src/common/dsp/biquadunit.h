...
public:	
	biquadunit();
	biquadunit(sub3_storage *storage);	
	...
	//static double calc_omega(double scfreq){ return (2*3.14159265358979323846) * min(0.499, 440*powf(2,scfreq)*samplerate_inv); }
	double calc_omega(double scfreq){ return (2*3.14159265358979323846) * 440*note_to_pitch((float)(12.f*scfreq))*dsamplerate_inv; }
	static double calc_omega_from_Hz(double Hz){ return (2*3.14159265358979323846) * Hz * dsamplerate_inv; }
	...
	void suspend(){ 
		reg0.d[0] = 0; reg1.d[0] = 0; reg0.d[1] = 0; reg1.d[1] = 0; 
		first_run = true; 
		a1.init_x87();
		a2.init_x87();
		b0.init_x87();
