...
: baseeffect(storage,fxdata,pd), band1(storage),band2(storage),lp1(storage),lp2(storage), hr_a(3,false), hr_b(3,true)
{
...
void distortion::setvars(bool init)
{
	if(init)
	{
		band1.coeff_peakEQ(band1.calc_omega(fxdata->p[1].val.f / 12.f), fxdata->p[2].val.f, fxdata->p[0].val.f);	
		band2.coeff_peakEQ(band2.calc_omega(fxdata->p[7].val.f / 12.f), fxdata->p[8].val.f, fxdata->p[6].val.f);	
...
void distortion::init_ctrltypes()
{
	...
	fxdata->p[0].set_name("Gain");		fxdata->p[0].set_type(ct_decibel);
	fxdata->p[1].set_name("Freq");		fxdata->p[1].set_type(ct_freq_audible);
	fxdata->p[2].set_name("BW");			fxdata->p[2].set_type(ct_bandwidth);			
	fxdata->p[3].set_name("High Cut");			fxdata->p[3].set_type(ct_freq_audible);
	
	fxdata->p[4].set_name("Drive");				fxdata->p[4].set_type(ct_decibel_narrow);
	fxdata->p[5].set_name("Feedback");			fxdata->p[5].set_type(ct_percent_bidirectional);		

	fxdata->p[6].set_name("Gain");		fxdata->p[6].set_type(ct_decibel);
	fxdata->p[7].set_name("Freq");		fxdata->p[7].set_type(ct_freq_audible);
	fxdata->p[8].set_name("BW");			fxdata->p[8].set_type(ct_bandwidth);		
	fxdata->p[9].set_name("High Cut");			fxdata->p[9].set_type(ct_freq_audible);	
	
	fxdata->p[10].set_name("Gain");		fxdata->p[10].set_type(ct_decibel_narrow);	
