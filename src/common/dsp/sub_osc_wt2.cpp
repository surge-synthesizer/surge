...
osc_WT2::osc_WT2(sub3_storage *storage, sub3_osc *oscdata, pdata *localcopy) : oscillator(storage,oscdata,localcopy)
{	
}

void osc_WT2::init(float pitch, bool is_display)
{
	memset(&Sub,0,sizeof(Sub));	

	ActiveSubOscs = limit_range(oscdata->p[6].val.i,1,wt2_suboscs-1);
	if(is_display) ActiveSubOscs = 1;
	...
	else 
	{
		...
		for(int i=0; i<ActiveSubOscs; i++)
		{
			...
			if(oscdata->retrigger.val.b)
				Sub.Pos[i] = (storage->WindowWT.size + ((storage->WindowWT.size * i)/ActiveSubOscs)) << 16;
			else
...
void osc_WT2::init_ctrltypes()
{
	oscdata->p[0].set_name("Shape");		oscdata->p[0].set_type(ct_percent);
	oscdata->p[1].set_name("Formant");		oscdata->p[1].set_type(ct_pitch);
	oscdata->p[2].set_name("Window");		oscdata->p[2].set_type(ct_wt2window);
	
	
	oscdata->p[5].set_name("Uni Spread");	oscdata->p[5].set_type(ct_oscspread);	
	oscdata->p[6].set_name("Uni Count");	oscdata->p[6].set_type(ct_osccountWT);	
}
void osc_WT2::init_default_values()
{
...
void osc_WT2::ProcessSubOscs(bool stereo)
{
	...
	int Table = limit_range((int)(float)(oscdata->wt.n_tables * localcopy[oscdata->p[0].param_id_in_scene].f), 0, (int)oscdata->wt.n_tables - 1);
	int FormantMul = (int)(float)(65536.f*note_to_pitch(localcopy[oscdata->p[1].param_id_in_scene].f));
	...
	{			
		...
		if(Sub.Table[so] >= oscdata->wt.n_tables) Sub.Table[so] = Table;	// TableID kanske inte ‰r valid l‰ngre om en ny wavetable laddats		
		...
		{													
			...
			else IOutputL[i] += ((iWin>>13) * (iWave>>13)) >> 6;
		}		
		Sub.Pos[so] = Pos;
	}	
#else
...
#endif
	{
		...
		{			
			...
			unsigned int MipMapA = 0;
			unsigned int MipMapB = 0;
			if(Sub.Table[so] >= oscdata->wt.n_tables) Sub.Table[so] = Table;	// TableID kanske inte ar valid langre om en ny wavetable laddats
			...
			{													
				...
				__m128i Wave = _mm_madd_epi16(
					_mm_load_si128(((__m128i*)sinctableI16 + MSPos)),
					_mm_loadu_si128((__m128i*)&WaveAdr[MPos]));

				__m128i Win = _mm_madd_epi16(
					_mm_load_si128(((__m128i*)sinctableI16 + WinSPos)),
				...
				else IOutputL[i] += (iWin[0] * iWave[0]) >> 6;
			}		
			Sub.Pos[so] = Pos;
		}	
	}
	...
	else
	{
		...
		{			
			...
			{													
				...
				__m64 Wave = _mm_add_pi32(_mm_madd_pi16( *WaveSincPtr, *WavePtr),_mm_madd_pi16( *(WaveSincPtr+1), *(WavePtr+1)));

				__m64 *WinSincPtr = (__m64*)sinctableI16 + (WinSPos<<1);
				__m64 *WinPtr = (__m64*)&WinAdr[WinPos];
				__m64 Win = _mm_add_pi32(_mm_madd_pi16( *WinSincPtr, *WinPtr),_mm_madd_pi16( *(WinSincPtr+1), *(WinPtr+1)));
...
void osc_WT2::process_block(float pitch, float drift, bool stereo, bool FM, float fmdepth)
{				
	memset(IOutputL,0,block_size_os*sizeof(int));
	if(stereo) memset(IOutputR,0,block_size_os*sizeof(int));
	...
	for(int l=0; l<ActiveSubOscs; l++) 
	{
		...
		float f = note_to_pitch((pitch - 57.f) + drift*Sub.DriftLFO[l][0] + Detune*(DetuneOffset + DetuneBias*(float)l));
		int Ratio = Float2Int(220.f * 32768.f * f * (float)(storage->WindowWT.size) * samplerate_inv);  // (65536.f*0.5f), 0.5 fˆr oversampling	
		Sub.Ratio[l] = Ratio;
	}		
	...
	else
	{
		...
		if(stereo)
		{
			for(int i=0; i<block_size_os; i+=4)
			{		
				_mm_store_ps(&output[i],_mm_mul_ps(_mm_cvtpi32x2_ps(*(__m64*)&IOutputL[i],*(__m64*)&IOutputL[i+2]),scale));				
				_mm_store_ps(&outputR[i],_mm_mul_ps(_mm_cvtpi32x2_ps(*(__m64*)&IOutputR[i],*(__m64*)&IOutputR[i+2]),scale));								
			}
		}
		else
		{
