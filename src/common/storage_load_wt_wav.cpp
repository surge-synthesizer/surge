...
void error_msg(char *c){}	// add messagebox?

#define uint32 unsigned int
#define int32 int

void sub3_storage::load_wt_wav(string filename, wavetable *wt)
{		
	...
	if (mmioRead(hmmio, (HPSTR)&waveFormat, sizeof(WAVEFORMATEXTENSIBLE)) != (LRESULT)sizeof(WAVEFORMATEXTENSIBLE))
	{
		/* Oops! */
	...
	if(!((waveFormat.Format.wFormatTag == WAVE_FORMAT_PCM)||(waveFormat.Format.wFormatTag == WAVE_FORMAT_IEEE_FLOAT))){
		error_msg("file	io: only WAVE_FORMAT_PCM and WAVE_FORMAT_IEEE_FLOAT wave-files are supported!");
		mmioClose(hmmio, 0);
		return;
	}
	...
	int32 WaveDataSamples = 8*WaveDataSize / (waveFormat.Format.wBitsPerSample*waveFormat.Format.nChannels);
	...
	if (mmioRead(hmmio,(HPSTR)loaddata, mmckinfoSubchunk.cksize) != (LRESULT)mmckinfoSubchunk.cksize)		// ACHTUNG!! här händer något bad!
	{
		/* Oops! */
	...
	if (mmioDescend(hmmio, &mmckinfoSubchunk, &mmckinfoParent, MMIO_FINDCHUNK))
	{
		...
		if (mmioRead(hmmio, (HPSTR)&inst_tag, mmckinfoSubchunk.cksize) != (LRESULT)mmckinfoSubchunk.cksize)
		{
	...
	if((waveFormat.Format.wFormatTag == WAVE_FORMAT_PCM)&&(waveFormat.Format.wBitsPerSample == 16))
	{
		assert(wh.n_samples * wh.n_tables * 2 <= WaveDataSize);
		wh.flags |= wtf_int16;
	} 
	else if((waveFormat.Format.wFormatTag == WAVE_FORMAT_IEEE_FLOAT)&&(waveFormat.Format.wBitsPerSample == 32))
	{
