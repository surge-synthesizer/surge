...
#else
   class vstlayer;
   typedef vstlayer PluginLayer;
#endif

class sub3_synth : public plugin
{	
public:
	// aligned stuff		
	_MM_ALIGN16 sub3_storage storage;
	_MM_ALIGN16 lipol_ps FX1,FX2,amp,amp_mute,send[2][2]; 		

	// methods
public:
	sub3_synth(PluginLayer* parent);
	virtual ~sub3_synth();
	...
	int get_n_inputs(){ return n_inputs; }
	int get_n_outputs(){ return n_outputs; }	
...
//protected:
   ...
   void onNRPN(int channel, int lsbNRPN, int msbNRPN, int lsbValue, int msbValue);
	...
	sub3_voice* get_unused_voice(int scene);
	void free_voice(sub3_voice*);
	sub3_voice *voices_array[2][max_voices];
	unsigned int voices_usedby[2][max_voices];	// 0 indicates no user, 1 is scene A & 2 is scene B

	bool setParameter01 (long index, float value, bool external=false, bool force_integer=false) final;	
...
//	float getParameter (long index);
	...
	modulation_routing* getModRouting(long ptag, long modsource);
	...
	void clear_osc_modulation(int scene,int entry);	// clear the modulation routings on the algorithm-specific sliders
...
public:		
   ...
   int mpeGlobalPitchBendRange = 0;
	...
	sub3_fx fxsync[8];	// used for synchronisation of parameter init
...
private:
   ...
   PluginLayer* _parent = nullptr;
	...
	modulation_controller mControlInterpolator[num_controlinterpolators];
	...
	modulation_controller* ControlInterpolator(int Idx);
	modulation_controller* AddControlInterpolator(int Idx, bool& AlreadyExisted);
};