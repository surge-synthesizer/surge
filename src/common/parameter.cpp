...
void get_prefix(char* txt, int ctrlgroup, int ctrlgroup_entry, int scene)
{
	...
	if(scene==2)		sprintf(txt,"b_%s",prefix);
	else if(scene==1)	sprintf(txt,"a_%s",prefix);
	else				sprintf(txt,"%s",prefix);	
}
...
void create_fullname(char* dn, char* fn, int ctrlgroup, int ctrlgroup_entry)
{
	...
	case 4:
		...
		break;
	case 5:
		if(ctrlgroup_entry)	sprintf(prefix,"FEG");
		else sprintf(prefix,"AEG");
		break;
	case 6:
		{
			int a = ctrlgroup_entry + 1 - ms_lfo1;
			if(a>6) sprintf(prefix,"SLFO%i",a - 6);
			else sprintf(prefix,"LFO%i",a);
		}
		break;
	case 7:
	...
	if (useprefix) sprintf(fn,"%s %s",prefix,dn);	
	else sprintf(fn,"%s",dn);	
}
...
parameter* parameter::assign(int id, int pid, const char* name, const char *dispname, int ctrltype, int posx, int posy, int scene, int ctrlgroup, int ctrlgroup_entry, bool modulateable, int ctrlstyle)
{		
	this->id = id;
	...
	posy_offset = 0;		
	if(scene) per_voice_processing = true;
	else per_voice_processing = false;	
	clear_flags();
	midictrl = -1;
	
	set_type(ctrltype);
	if(valtype == vt_float) val.f = val_default.f;
...
void parameter::set_type(int ctrltype)
{
	...
	case ct_fxtype:
		...
		// affect_other_parameters = true;	// kan inte addas förrän den har en custom controltype
		break;	
	case ct_fxbypass:
		valtype = vt_int;
		val_min.i = 0;
...
char* parameter::get_storage_value(char *str)
{
	switch(ctrltype)
	{
/*	case ct_amplitude:  // blir strul för effekter eftersom ctrltype inte är definierad då värdet laddas
		sprintf(str,"%f",amp_to_db(val.f));
		break;*/
	default:
		switch(valtype)
		{
...
void parameter::get_display(char *txt, bool external, float ef)
{
	...
	{
	case vt_float:
		if(external) f = ef*(val_max.f - val_min.f) + val_min.f;
		else f = val.f;
		switch(ctrltype)
		{
		case ct_portatime:
		...
		break;
	case vt_int:
		if(external) i = (int)((1/0.99)*(ef-0.005)*(float)(val_max.i - val_min.i) + 0.5) + val_min.i;
		else i = val.i;
		switch(ctrltype)
		{
		case ct_osctype:
