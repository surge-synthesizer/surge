...
void halfrate_stereo::process_block_D2(float * floatL, float * floatR, int nsamples, float* outL, float* outR)
{
	...
	for(int k=0; k<nsamples; k+=8)
	{
		...
		aR = _mm_movehl_ps(aR,o[k+1]); aR = _mm_add_ss(aR,tR0);
		...
		bR = _mm_movehl_ps(aR,o[k+3]); bR = _mm_add_ss(bR,tR0);
		...
		cR = _mm_movehl_ps(cR,o[k+5]); cR = _mm_add_ss(cR,tR0);
		...
		dR = _mm_movehl_ps(dR,o[k+7]); dR = _mm_add_ss(dR,tR0);
...
void halfrate_stereo::process_block_U2(float * floatL_in, float * floatR_in, float * floatL, float * floatR, int nsamples)
{
	__m128 * L = (__m128*) floatL;
	__m128 * R = (__m128*) floatR;
