...
// patch. Each block is given a unique ID, generated by the patch. The IDs are
// saved and loaded by the patch for the pieces that use them (for example, FX
// storages). Then the patch save/load matches the blocks' IDs to what needs
// them and fills them in.
//
// Saves and loads will preserve ordering within any arrays of
// ArbitraryBlockStorage.
...
class SurgePatch
{
  public:
    ...
    // Next available ID for block storage, incrementing monotonically.
    std::atomic<std::uint16_t> block_id;
