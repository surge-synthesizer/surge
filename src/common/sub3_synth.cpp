...
: hpA(&storage), hpB(&storage), _parent(parent)
{
	...
	for(int sc=0; sc<2; sc++)
	{
		...
		for(int i=0; i<n_modsources; i++) storage.getPatch().scene[sc].modsources[i] = 0;
		...
		for(int l=0; l<n_lfos_scene; l++)
		{
			...
			((modulation_lfo*)storage.getPatch().scene[sc].modsources[ms_slfo1 + l])->assign(&storage,&storage.getPatch().scene[sc].lfo[n_lfos_voice + l],storage.getPatch().scenedata[sc],0,&storage.getPatch().stepsequences[sc][n_lfos_voice + l]);		
		}		
	}	
	for(int i=0; i<n_customcontrollers; i++) 
	{
		storage.getPatch().scene[0].modsources[ms_ctrl1+i] = new modulation_controller();
		storage.getPatch().scene[1].modsources[ms_ctrl1+i] = storage.getPatch().scene[0].modsources[ms_ctrl1+i];
	}
	...
	for(int i=0; i<8; i++) vu_peak[i] = 0.f;
	learn_param = -1;
	learn_custom = -1;
...
sub3_synth::~sub3_synth()
{
	...
	{		
		...
		for(int i=0; i<n_lfos_scene; i++) delete storage.getPatch().scene[sc].modsources[ms_slfo1+i];			
	}
	for(int i=0; i<n_customcontrollers; i++) delete storage.getPatch().scene[0].modsources[ms_ctrl1+i];
	for(int i=0; i<8; i++) _aligned_free(fx[i]);
}
...
int sub3_synth::calc_channelmask(int channel, int key)
{
   ...
   int channelmask = channel;
	...
	if((channel == 0)||(channel > 2)||mpeEnabled)
	{
		...
		case sm_single:		
			//	case sm_morph:
			if(storage.getPatch().scene_active.val.i == 1) channelmask = 2;
			else channelmask = 1;
			break;
		case sm_dual:		
			channelmask = 3;
			break;
		case sm_split:
			if(key < storage.getPatch().splitkey.val.i) channelmask = 1;
			else channelmask = 2;
			break;	
		}
	}
...
void sub3_synth::play_note(char channel, char key, char velocity, char detune)
{
   ...
   // MIDI Channel 3 plays B
	...
	if(channelmask & 1) play_voice(0,channel,key,velocity,detune);
	if(channelmask & 2) play_voice(1,channel,key,velocity,detune);
...
{		
	...
	if(max_age_release) (*max_released)->uber_release();
	else if(max_age) (*max_playing)->uber_release();
}
...
void sub3_synth::play_voice(int scene, char channel, char key, char velocity, char detune)
{
	...
	{
		for(int l=0; l<n_lfos_scene; l++) storage.getPatch().scene[scene].modsources[ms_slfo1 + l]->attack();
	}	
	
	//int excess_voices = max(0,(int)get_non_ur_voices(scene) - storage.getPatch().polylimit.val.i + 1);	// +1 för det kommer ju en ny voice här
	int excess_voices = max(0,(int)get_non_ur_voices(scene) - storage.getPatch().polylimit.val.i + 1);	// +1 för det kommer ju en ny voice här
	...
	case pm_latch:
		{
            ...
            int mpeMainChannel = getMpeMainChannel(channel, key);
				...
				voices[scene].push_back(nvoice);						
				if ((storage.getPatch().scene[scene].polymode.val.i == pm_mono_fp) && !glide) storage.last_key[scene] = key;
				new(nvoice) sub3_voice(
               &storage,
               &storage.getPatch().scene[scene],
               storage.getPatch().scenedata[scene],
	...
	case pm_mono_st_fp:		
		{
			...
			for (iter=voices[scene].begin(); iter != voices[scene].end(); iter++)
			{
				...
				{
					if(v->state.scene_id == scene) v->uber_release();		// make this optional for poly legato
				}
			}
			if (!found_one)
			{
...
{	
	...
	//if(channelmask&1)
	{
		if (!channelState[channel].hold) release_note_postholdcheck(0,channel,key,velocity);	
		else holdbuffer[0].push_back(key);	// hold pedal is down, add to bufffer
	}
	//if(channelmask&2)
	{
		if (!channelState[channel].hold) release_note_postholdcheck(1,channel,key,velocity);	
		else holdbuffer[1].push_back(key);	// hold pedal is down, add to bufffer
	}
}
...
{	
	...
	for(int s=0; s<2; s++)
	{
		...
		for (iter=voices[scene].begin(); iter != voices[scene].end(); iter++)
		{
			...
			{
				if(v->state.scene_id == 0) hikey = storage.getPatch().splitkey.val.i - 1;
				else lowkey = storage.getPatch().splitkey.val.i;
			}

			switch(storage.getPatch().scene[v->state.scene_id].polymode.val.i)
			{			
			case pm_poly:
				if((v->state.key == key)&&(v->state.channel == channel)) v->release();			
				break;
			case pm_mono:
			case pm_mono_fp:	
			case pm_latch:
				{
					if((v->state.key == key)&&(v->state.channel == channel)) 
					{
						...
						if(!do_switch) 
						{
							if(storage.getPatch().scene[v->state.scene_id].polymode.val.i != pm_latch) v->release();
						}
						else
						{
							// confirm that no notes are active
							v->uber_release();
							if(get_non_ur_voices(scene) == 0) play_voice(scene,channel,k,velocity,channelState[channel].keyState[k].lastdetune);
						}
					}
				}
				break;			
	...
	//for(int scene=0; scene<2; scene++)
	{
		if(get_non_released_voices(scene) == 0)
		{
			for(int l=0; l<n_lfos_scene; l++) storage.getPatch().scene[scene].modsources[ms_slfo1 + l]->release();
		}
	}
}
...
void sub3_synth::pitch_bend(char channel, int value)
{
   ...
   else
   {
      ...
      ((modulation_controller*)storage.getPatch().scene[0].modsources[ms_pitchbend])->set_target(storage.pitch_bend);
      ((modulation_controller*)storage.getPatch().scene[1].modsources[ms_pitchbend])->set_target(storage.pitch_bend);
   }
}
void sub3_synth::channel_aftertouch(char channel, int value)
{
   ...
   {
      ((modulation_controller*)storage.getPatch().scene[0].modsources[ms_aftertouch])->set_target(fval);
      ((modulation_controller*)storage.getPatch().scene[1].modsources[ms_aftertouch])->set_target(fval);
   }
}
...
void sub3_synth::channel_controller(char channel, int cc, int value)
{
	...
	// spara all möjliga NRPN & RPN's i ett short-array.. blir endast 128kb eller nåt av av det ändå.. 
	switch(cc)
	{
	case 0:
		CC0 = value;
		return;
	case 1:
		((modulation_controller*)storage.getPatch().scene[0].modsources[ms_modwheel])->set_target(fval);
		((modulation_controller*)storage.getPatch().scene[1].modsources[ms_modwheel])->set_target(fval);
		break;
	case 6:
      if (channelState[channel].nrpn_last)
      {
         ...
         onNRPN(
            channel,
            channelState[channel].nrpn[0],
            channelState[channel].nrpn[1],
            channelState[channel].nrpn_v[0],
            channelState[channel].nrpn_v[1]);
      }
      else
      {
         ...
         onRPN(
            channel,
            channelState[channel].rpn[0],
            channelState[channel].rpn[1],
            channelState[channel].rpn_v[0],
            channelState[channel].rpn_v[1]);
      }
		break;
	case 38:
		if (channelState[channel].nrpn_last)	channelState[channel].nrpn_v[0] = value;
		else channelState[channel].rpn_v[0] = value;
		break;
	case 64:
		{	
			channelState[channel].hold = value>63;	// check hold pedal
			if (channelmask&1) purge_holdbuffer(0);
			if (channelmask&2) purge_holdbuffer(1);
			return;
		}
   ...
   case 127: // POLY ON
	...
	for(int i=0; i<n_customcontrollers; i++)
	{
		...
		{			
			((modulation_controller*)storage.getPatch().scene[0].modsources[ms_ctrl1 + i])->set_target01(fval);
	...
	if (learn_param >= 0) 
	{
		...
		{
			int a = learn_param;
			if (learn_param >= (n_global_params+n_scene_params)) a -= n_scene_params;
	...
	{		
		...
		{			
			...
			while(j<7)
			{
				if ((refresh_ctrl_queue[j] > -1) && (refresh_ctrl_queue[j] != i)) j++;	
				else break;
			}
...
{		
	...
	while(1)
	{
		if(iter == holdbuffer[scene].end()) break;
		z = *iter;
...
{	
	...
	{
		((modulation_controller*)storage.getPatch().scene[0].modsources[ms_ctrl1+index-metaparam_offset])->set_target01(value,true);
		return false;
	}
...
{	
	...
	{				
		if((fxsync[s].type.val.i != storage.getPatch().fx[s].type.val.i) || force_reload_all)
		{
			...
			if (/*!force_reload_all && */storage.getPatch().fx[s].type.val.i) memcpy(&storage.getPatch().fx[s].p,&fxsync[s].p,sizeof(parameter)*n_fx_params);

			fx[s] = spawn_effect(storage.getPatch().fx[s].type.val.i, &storage, &storage.getPatch().fx[s], storage.getPatch().globaldata);

			if (fx[s])
			{						
				fx[s]->init_ctrltypes();
				if(initp) fx[s]->init_default_values();
				/*for(int j=0; j<n_fx_params; j++) 
				{	
					storage.getPatch().globaldata[storage.getPatch().fx[s].p[j].id].f = storage.getPatch().fx[s].p[j].val.f;					
				}*/
...
bool sub3_synth::load_oscalgos()
{
	for(int s=0; s<2; s++)
	{
		...
		{				
			...
			{
				storage.getPatch().scene[s].osc[i].type.val.i = storage.getPatch().scene[s].osc[i].queue_type;
			...
			if(e)
			{
				for(int k=0; k<n_osc_params; k++)
				{
					...
					{
						if(e->QueryDoubleAttribute(lbl,&d) == TIXML_SUCCESS) storage.getPatch().scene[s].osc[i].p[k].val.f = (float)d;
					}
					else
					{
						if(e->QueryIntAttribute(lbl,&j) == TIXML_SUCCESS) storage.getPatch().scene[s].osc[i].p[k].val.i = j;
					}
				}
...
{
	if(!modsource) return false;
	if (!(ptag<storage.getPatch().param_ptr.size())) return false;
	parameter *p = storage.getPatch().param_ptr[ptag];

	if (!p->modulateable) return false;	
	if (p->valtype != ((valtypes)vt_float)) return false;
	if(!p->per_voice_processing && !is_scenelevel(modsource)) return false;
	if((modsource == ms_keytrack)&&(p == &storage.getPatch().scene[0].pitch)) return false;
	if((modsource == ms_keytrack)&&(p == &storage.getPatch().scene[1].pitch)) return false;
	if((p->ctrlgroup == 6)&&(p->ctrlgroup_entry >= ms_lfo1)&&!can_modulate_modulators(modsource)) return false;
	if((p->ctrlgroup == 6)&&(p->ctrlgroup_entry == modsource)) return false;
	if((p->ctrlgroup == 6)&&(p->ctrlgroup_entry >= ms_slfo1)&&(!is_scenelevel(modsource))) return false;
	if((p->ctrlgroup == 5)&&!can_modulate_modulators(modsource)) return false;
	return true;
}

modulation_routing* sub3_synth::getModRouting(long ptag, long modsource)
{
	if (!isValidModulation(ptag,modsource)) return 0;
	...
	if(!scene)
	{
		if(is_scenelevel(modsource)) modlist = &storage.getPatch().modulation_global;
		else return 0;
	}
	else
	{
		if(is_scenelevel(modsource)) modlist = &storage.getPatch().scene[scene-1].modulation_scene;
		else modlist = &storage.getPatch().scene[scene-1].modulation_voice;			
	}
	...
	for(int i=0; i<n; i++)
	{
		if((modlist->at(i).destination_id == id) &&
			(modlist->at(i).source_id == modsource))
		{
			return &modlist->at(i);
		}	
	}
...
float sub3_synth::getModDepth(long ptag,long modsource)
{
	...
	if(p && p->extend_range)  d = p->get_extended(d);
	return d;
}
...
bool sub3_synth::isActiveModulation(long ptag, long modsource)
{
	...
	if(getModRouting(ptag,modsource)) return true;
	return false;
}
...
{		
	...
	if(scene_p) md_id = storage.getPatch().param_ptr[ptag]->param_id_in_scene;
	...
	for(int j=0; j<3; j++)
	{
		...
		if((scene_p && (j>0)) || (!scene_p && (j==0)))
		{
			...
			for(int i=0; i<n; i++)
			{			
				if (modlist->at(i).destination_id == md_id) return true;
			}
		}
	}
	return false;
...
{	
	...
	for(int i=0; i<n_modsources; i++) modsourceused[i] = false;
...
{
	if (!isValidModulation(ptag,modsource)) return 0.0f;
...
{	
	if (!isValidModulation(ptag,modsource)) return;
	int scene = storage.getPatch().param_ptr[ptag]->scene;	
...
{ 	
	...
	if(index>=metaparam_offset) return storage.getPatch().scene[0].modsources[ms_ctrl1+index-metaparam_offset]->get_output01();
	if(index<storage.getPatch().param_ptr.size()) return storage.getPatch().param_ptr[index]->get_value_f01();	
	return 0.f; 
}
...
{	
	if ((index >= 0) && (index<storage.getPatch().param_ptr.size()))
	{
		...
		pm.fdefault = storage.getPatch().param_ptr[index]->get_default_value_f01();  //storage.getPatch().param_ptr[index]->val_default.f;
		...
		{
			pm.clump += storage.getPatch().param_ptr[index]->ctrlgroup + (storage.getPatch().param_ptr[index]->scene-1)*6;
			if(storage.getPatch().param_ptr[index]->ctrlgroup == 0) pm.clump++;
		}			
	}	
	else if(index >= metaparam_offset)
	{
		...
		pm.fdefault = 0.5f;
		pm.hide = false;
		pm.meta = false; // ironiskt eftersom det Šr metaparameters, men dom pŒverkar inga andra sliders
		pm.expert = false;
		pm.clump = 1;
	}
}
...
{
   if (index<0) return 0.f;
   if (index >= metaparam_offset) return storage.getPatch().scene[0].modsources[ms_ctrl1 + index - metaparam_offset]->get_output();
   if (index<storage.getPatch().param_ptr.size()) return storage.getPatch().param_ptr[index]->get_value_f01();
   return 0.f;
}
...
{
   if (index<0) return 0.f;
   if (index<storage.getPatch().param_ptr.size()) return storage.getPatch().param_ptr[index]->normalized_to_value(value);
   return 0.f;
}
...
{
   if (index<0) return 0.f;
   if (index<storage.getPatch().param_ptr.size()) return storage.getPatch().param_ptr[index]->value_to_normalized(value);
   return 0.f;
}
...
void sub3_synth::process_threadunsafe_operations()
{
	if(!audio_processing_active)
	{
		...
		if (load_fx_needed) load_fx(false,false);
...
{		
	...
	{		
		if (!playA || release_anyway[0]) release_scene(0);
		release_if_latched[0] = false;
		release_anyway[0] = false;
	}
	if(release_if_latched[1])
	{		
		if (!playB || release_anyway[1]) release_scene(1);
	...
	if(playA && (storage.getPatch().scene[0].polymode.val.i == pm_latch) && voices[0].empty()) play_note(1,60,100,0);
	if(playB && (storage.getPatch().scene[1].polymode.val.i == pm_latch) && voices[1].empty()) play_note(2,60,100,0);
	...
	storage.getPatch().copy_globaldata(storage.getPatch().globaldata);		// suger ganska mkt cpu i debug mode
	if (playA) storage.getPatch().copy_scenedata(storage.getPatch().scenedata[0],0);		// -""-	
	if (playB) storage.getPatch().copy_scenedata(storage.getPatch().scenedata[1],1);
...
//	if(sm == sm_morph) storage.getPatch().do_morph();
	...
	for(int s=0; s<2; s++)
	{
		if(((s == 0)&&playA)||((s == 1)&&playB))
		{
			...
			//for(int i=0; i<n_lfos_scene; i++) storage.getPatch().scene[s].modsources[ms_slfo1+i]->process_block();
			...
			for(int i=0; i<n; i++)
			{
				...
				if(storage.getPatch().scene[s].modsources[src_id])
				{
					...
					storage.getPatch().scenedata[s][dst_id].f += depth * storage.getPatch().scene[s].modsources[src_id]->output;
				}
			}
	...
	if(fx_suspend_bitmask)
	{
		for(int i=0; i<8; i++)
		{
			if(((1<<i) & fx_suspend_bitmask) && fx[i])	fx[i]->suspend();
		}
		fx_suspend_bitmask = 0;
	}
}
...
void sub3_synth::process()
{
	...
	{		
		if(fx[0] && !(storage.getPatch().fx_disable.val.i & (1<<0))) sc_a = fx[0]->process_ringout(sceneout[0][0],sceneout[0][1],sc_a);
		if(fx[1] && !(storage.getPatch().fx_disable.val.i & (1<<1))) sc_a = fx[1]->process_ringout(sceneout[0][0],sceneout[0][1],sc_a);
		if(fx[2] && !(storage.getPatch().fx_disable.val.i & (1<<2))) sc_b = fx[2]->process_ringout(sceneout[1][0],sceneout[1][1],sc_b);
		if(fx[3] && !(storage.getPatch().fx_disable.val.i & (1<<3))) sc_b = fx[3]->process_ringout(sceneout[1][0],sceneout[1][1],sc_b);
	}
	...
	if(fx_bypass == fxb_all_fx)
	{
		...
		{
			send[0][0].MAC_2_blocks_to(sceneout[0][0],sceneout[0][1],fxsendout[0][0],fxsendout[0][1],block_size_quad);
			send[0][1].MAC_2_blocks_to(sceneout[1][0],sceneout[1][1],fxsendout[0][0],fxsendout[0][1],block_size_quad);
			send1 = fx[4]->process_ringout(fxsendout[0][0],fxsendout[0][1],sc_a || sc_b);
			FX1.MAC_2_blocks_to(fxsendout[0][0],fxsendout[0][1],output[0],output[1],block_size_quad);
		}
		if(fx[5] && !(storage.getPatch().fx_disable.val.i & (1<<5)))
		{
			send[1][0].MAC_2_blocks_to(sceneout[0][0],sceneout[0][1],fxsendout[1][0],fxsendout[1][1],block_size_quad);
			send[1][1].MAC_2_blocks_to(sceneout[1][0],sceneout[1][1],fxsendout[1][0],fxsendout[1][1],block_size_quad);
			send2 = fx[5]->process_ringout(fxsendout[1][0],fxsendout[1][1],sc_a || sc_b);
			FX2.MAC_2_blocks_to(fxsendout[1][0],fxsendout[1][1],output[0],output[1],block_size_quad);
		}
	}
	...
	if((fx_bypass == fxb_all_fx)||(fx_bypass == fxb_no_sends))
	{
		...
		if(fx[6] && !(storage.getPatch().fx_disable.val.i & (1<<6))) glob = fx[6]->process_ringout(output[0],output[1],glob);
		if(fx[7] && !(storage.getPatch().fx_disable.val.i & (1<<7))) glob = fx[7]->process_ringout(output[0],output[1],glob);
	}
