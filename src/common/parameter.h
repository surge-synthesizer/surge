...
public:	
	parameter();
	parameter* assign(int id, int pid, const char* name, const char *dispname, int ctrltype, int posx, int posy, int scene=0, int ctrlgroup=0, int ctrlgroup_entry=0, bool modulateable=true, int ctrlstyle=cs_off);
	bool can_temposync();
...
//	void morph(parameter *b, float x);
   ...
   float value_to_normalized(float value);
	...
	float get_modulation_f01(float mod);		// used by the gui to get the position of the modulated handle
	float set_modulation_f01(float v);			// used by the gui to set the modulation to match the position of the modulated handle
