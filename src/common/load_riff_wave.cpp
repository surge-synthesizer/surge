...
bool sample::load_riff_wave_mk2(const char *fname)
{
	...
	if(!((waveFormat.Format.wFormatTag == WAVE_FORMAT_PCM)||(waveFormat.Format.wFormatTag == WAVE_FORMAT_IEEE_FLOAT))){
		write_log("file	io: only WAVE_FORMAT_PCM and WAVE_FORMAT_IEEE_FLOAT wave-files are supported!");
		mmioClose(hmmio, 0);
		return false;
	}
	...
	int WaveDataSamples = 8*WaveDataSize / (waveFormat.Format.wBitsPerSample*waveFormat.Format.nChannels);
	...
	/* if the read has been succesfull this far, we can delete the old sample (if it exists) and replace it with a new one */
...
//	this->sample_start = 0;
	...
	unsigned int i;
	for(i=0; i<samplesizewithmargin; i++) sample_data[i] = 0.0f;	
	...
	if(waveFormat.Format.wFormatTag == WAVE_FORMAT_PCM)
	{
		...
		} else if(waveFormat.Format.wBitsPerSample == 16)
		{
			...
			for(i=0; i<sample_length; i++)
			{
				int value = (loaddata[((i*channels)<<1)+1] << 8) + loaddata[((i*channels)<<1)];
		...
		} else if(waveFormat.Format.wBitsPerSample == 24)
		{
			unsigned int i;
			{
				for(i=0; i<sample_length; i++)
				{
					...
					//int value = (loaddata[(3*i*channels)+2] << 16) + (loaddata[(3*i*channels)+1] << 8) + loaddata[(3*i*channels)];
