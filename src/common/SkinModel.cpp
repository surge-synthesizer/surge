...
namespace Surge
{
   namespace Skin
   {
      namespace FEG {
         ...
         Connector fegPanel = Connector( "feg.panel", 459, 267, Connector::GROUP );
      ...
      namespace AEG {
         Connector aegPanel = Connector( "aeg.panel", 609, 267, Connector::GROUP );
      ...
      namespace FX { // DONE except VU but still with the terrible API
         Connector fxJog = Connector( "fx.preset.prevnext", 860, 212, Connector::JOG_FX ).asJogPlusMinus();
         Connector fxPreset = Connector( "fx.preset.name", 762, 212, 95, 12, Connector::CUSTOM, Connector::FXPRESET_LABEL );
         Connector fxSelector = Connector( "fx.selector", 767, 79, 123, 51, Connector::CUSTOM, Connector::FX_SELECTOR );
         Connector fxtype = Connector( "fx.type", 761, 194, 133, 18, Connector::FXMENU );
      ...
      namespace LFO { // DONE
         ...
         Connector lfoLabel = Connector( "lfo.name", 5, 485, 11, 83, Connector::CUSTOM, Connector::LFO_LABEL );
      ...
      namespace Osc { // All Done
         ...
         Connector osctype = Connector( "osc.type", 98, 194, 45, 18, Connector::OSCMENU );

         Connector oscParamPanel = Connector( "osc.param.panel", 6, 212, Connector::GROUP );
      ...
      namespace OtherControls {
         ...
         Connector patchCategoryJog = Connector( "controls.category.prevnext", 157, 42, Connector::JOG_PATCHCATEGORY).asJogPlusMinus();
         Connector patchJog = Connector( "controls.patch.prevnext", 246, 42, Connector::JOG_PATCH ).asJogPlusMinus();
         Connector patchStore = Connector( "controls.patch.store", 510, 42, 37, 12, Connector::HSWITCH2, Connector::STORE_PATCH)
         ...
         Connector statusPanel = Connector( "controls.status.panel", 562, 12, Connector::GROUP );
         ...
         Connector statusTune = Connector( "controls.status.tune", 0, 15, 31, 12, Connector::SWITCH, Connector::STATUS_TUNE )
             .withBackground( IDB_TUNE_BUTTON ).inParent( "controls.status.panel");
         Connector statusZoom = Connector( "controls.status.zoom", 0, 30, 31, 12, Connector::SWITCH, Connector::STATUS_ZOOM )
