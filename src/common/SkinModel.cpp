...
Connector splitpoint =
    Connector("scene.splitpoint", 100, 11, 43, 14, Connector::NUMBERFIELD)
        .withBackground(IDB_NUMFIELD_POLY_SPLIT)
        .withProperty(Connector::TEXT_COLOR, "scene.split_poly.text")
        .withProperty(Connector::TEXT_HOVER_COLOR, "scene.split_poly.text.hover");
...
Connector keytrack_root =
    Connector("scene.keytrack_root", 311, 266, 43, 14, Connector::NUMBERFIELD)
...
Connector osc_select =
    Connector("osc.select", 66, 69, 75, 13, Connector::HSWITCH2, Connector::OSCILLATOR_SELECT)
        .withHSwitch2Properties(IDB_OSC_SELECT, 3, 1, 3);
Connector osc_display =
    Connector("osc.display", 4, 81, 141, 99, Connector::CUSTOM, Connector::OSCILLATOR_DISPLAY);

Connector keytrack =
    Connector("osc.keytrack", 4, 180, 45, 9, Connector::SWITCH).withBackground(IDB_OSC_KEYTRACK);
Connector retrigger =
    Connector("osc.retrigger", 51, 180, 45, 9, Connector::SWITCH).withBackground(IDB_OSC_RETRIGGER);
...
Connector f2_link_resonance =
    Connector("filter.f2_link_resonance", 734, 239, 12, 18, Connector::SWITCH)
...
Connector waveshaper_type =
    Connector("filter.waveshaper_type", 388, 311, 28, 51, Connector::HSWITCH2)
...
Connector fx_selector =
    Connector("fx.selector", 767, 79, 123, 51, Connector::CUSTOM, Connector::FX_SELECTOR);
Connector fx_type = Connector("fx.type", 761, 194, 133, 18, Connector::FXMENU);

Connector fx_preset =
    Connector("fx.preset.name", 762, 212, 95, 12, Connector::CUSTOM, Connector::FXPRESET_LABEL);
...
Connector lfo_title_label =
    Connector("lfo.title", 6, 489, 11, 83, Connector::CUSTOM, Connector::LFO_LABEL);
Connector lfo_presets =
    Connector("lfo.presets", 6, 484, 13, 11, Connector::SWITCH, Connector::LFO_MENU)
...
Connector unipolar =
    Connector("lfo.unipolar", 172, 546, 51, 14, Connector::SWITCH).withBackground(IDB_LFO_UNIPOLAR);
...
Connector mseg_editor =
    Connector("lfo.mseg_editor", 597, 484, 11, 11, Connector::SWITCH, Connector::MSEG_EDITOR_OPEN)
...
Connector surge_menu =
    Connector("controls.surge_menu", 844, 550, 50, 15, Connector::HSWITCH2, Connector::SURGE_MENU)
        .withProperty(Connector::BACKGROUND, IDB_MAIN_MENU)
        .withProperty(Connector::DRAGGABLE_HSWITCH, false);

Connector patch_browser = Connector("controls.patch_browser", 157, 12, 390, 28, Connector::CUSTOM,
...
Connector patch_store =
    Connector("controls.patch.store", 510, 42, 37, 12, Connector::HSWITCH2, Connector::STORE_PATCH)
...
Connector status_mpe =
    Connector("controls.status.mpe", 0, 0, 31, 12, Connector::SWITCH, Connector::STATUS_MPE)
        .withBackground(IDB_MPE_BUTTON)
        .inParent("controls.status.panel");
Connector status_tune =
    Connector("controls.status.tune", 0, 15, 31, 12, Connector::SWITCH, Connector::STATUS_TUNE)
        .withBackground(IDB_TUNE_BUTTON)
        .inParent("controls.status.panel");
Connector status_zoom =
    Connector("controls.status.zoom", 0, 30, 31, 12, Connector::SWITCH, Connector::STATUS_ZOOM)
        .withBackground(IDB_ZOOM_BUTTON)
        .inParent("controls.status.panel");

Connector vu_meter =
    Connector("controls.vu_meter", 763, 15, 123, 13, Connector::VU_METER, Connector::MAIN_VU_METER);
...
Connector store_patch_dialog = Connector("controls.patch.store.window", 157, 57, 390, 143,
                                         Connector::CUSTOM, Connector::STORE_PATCH_DIALOG);
