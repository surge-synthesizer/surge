...
struct keywords : pegtl::sor<bin_op> {};
...
struct value : pegtl::sor<quoted_value_in_quotes, plain_value> {};
...
std::unique_ptr<PatchDBQueryParser::Token> PatchDBQueryParser::parseQuery(const std::string &q)
{
    ...
    std::cout << "TESTING WITH [" << q << "]" << std::endl;
    ...
    if (root)
    {
        ...
        if (root->children.size() == 1)
            return treeToToken(*(root->children[0]));
