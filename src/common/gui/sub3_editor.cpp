...
void sub3_editor::idle()
{
	...
	{	
		if(aboutbox && (aboutbox->getValue() > 0.5f)) return;
		...
		if(v && dynamic_cast<CControl*>(v) != nullptr)
		{
			...
			{				
				...
				{								
					((gui_modsrcbutton*)gui_modsrc[i])->update_rt_vals(synth->isActiveModulation(ptag,i),0,synth->isModsourceUsed(i));									
				}	
      ...
      if (synth->storage.getPatch().scene[current_scene].osc[current_osc].wt.refresh_display)
      {
		...
		if(queue_refresh || synth->refresh_editor)
		{
			...
			if(frame)
			{		
				if (synth->patch_loaded) mod_editor = false;
		...
		for(int i=0; i<8; i++)
		{
			if(synth->refresh_parameter_queue[i] >= 0)
			{
				...
				else if((j >= metaparam_offset)&&(j < (metaparam_offset + n_customcontrollers)))
				{
					...
					gui_modsrc[ms_ctrl1 + cc]->setValue(((modulation_controller*)synth->storage.getPatch().scene[0].modsources[ms_ctrl1+i])->get_target01());
				}
			}
		}
		for(int i=0; i<n_customcontrollers; i++)
		{
			if(((modulation_controller*)synth->storage.getPatch().scene[0].modsources[ms_ctrl1+i])->has_changed(true))
			{
				gui_modsrc[ms_ctrl1 + i]->setValue(((modulation_controller*)synth->storage.getPatch().scene[0].modsources[ms_ctrl1+i])->get_target01());
			}
		}
...
void sub3_editor::refresh_mod()
{
	...
	{					
		int state = 0;				
		if (i == modsource) state = mod_editor?2:1;
		if (i == modsource_editor) state |= 4;
...
CRect positionForModulationGrid(int entry)
{
   ...
   if(is_customcontroller(entry)) r.bottom += 8;
...
{	
   ...
   getFrame()->registerKeyboardHook(this);
	...
	{		
		...
		CControl *oscswitch = new gui_hswitch(rect,this,tag_osc_select,3,13,1,3,getSurgeBitmap(IDB_OSCSELECT),nopoint);									
	...
	for(int k=1; k<n_modsources; k++)
	{
      CRect r = positionForModulationGrid(k);
		...
		int state = 0;				
		if (k == modsource) state = mod_editor?2:1;
		if (k == modsource_editor) state |= 4;
		...
		{
			((gui_modsrcbutton*)gui_modsrc[k])->setlabel(synth->storage.getPatch().CustomControllerLabel[k-ms_ctrl1]);
			((gui_modsrcbutton*)gui_modsrc[k])->set_ismeta(true);					
			((gui_modsrcbutton*)gui_modsrc[k])->setBipolar(synth->storage.getPatch().scene[0].modsources[k]->is_bipolar());
			gui_modsrc[k]->setValue(((modulation_controller*)synth->storage.getPatch().scene[0].modsources[k])->get_target01());
		}
		else
      {
	...
	/*// Comments
	{
		...
		CTextLabel *Comments = new CTextLabel(CommentsRect,synth->storage.getPatch().comment.c_str());		
	...
	//CRect(12,62,140,159)
	oscdisplay = new gui_wavedisplay(CRect(6,81,142,180),&synth->storage.getPatch().scene[synth->storage.getPatch().scene_active.val.i].osc[current_osc],&synth->storage);		
	frame->addView(oscdisplay);	

	// 150*b - 16 = 434 (b=3)
	patchname = new gui_patchname(CRect(156,11,591,11+28),this,tag_patchname,&synth->storage);
	((gui_patchname*)patchname)->setLabel(synth->storage.getPatch().name);
	((gui_patchname*)patchname)->setCategory(synth->storage.getPatch().category);
	...
	gui_hswitch *mp_cat = new gui_hswitch(CRect(157,41,157+37,41+12),this,tag_mp_category,2,12,1,2,getSurgeBitmap(IDB_BUTTON_MINUSPLUS),nopoint,false);
	frame->addView(mp_cat);

	gui_hswitch *mp_patch = new gui_hswitch(CRect(242,41,242+37,41+12),this,tag_mp_patch,2,12,1,2,getSurgeBitmap(IDB_BUTTON_MINUSPLUS),nopoint,false);
	frame->addView(mp_patch);

	gui_hswitch *b_store = new gui_hswitch(CRect(591-37,41,591,41+12),this,tag_store,1,12,1,1,getSurgeBitmap(IDB_BUTTON_STORE),nopoint,false);
	frame->addView(b_store);	

	memset(param,0,512*sizeof(void*));
	int i=0;
	vector<parameter*>::iterator iter;		
	for (iter=synth->storage.getPatch().param_ptr.begin(); iter != synth->storage.getPatch().param_ptr.end(); iter++)
	{
		parameter *p = *iter;

		if((p->posx != 0) && ((p->scene == (current_scene+1))||(p->scene == 0)) && is_visible(p->ctrlgroup,p->ctrlgroup_entry) && (p->ctrltype != ct_none))
		{
			...
			case ct_filtertype:
				{
					...
					CControl *hsw = new gui_hswitch(rect,this,p->id+start_paramtags,10,18,1,10,getSurgeBitmap(IDB_FILTERBUTTONS),nopoint,true);									
					rect(3,0,124,14);
			...
			case ct_filtersubtype:
				{
					...
					CControl *hsw = new gui_switch(rect,this,p->id+start_paramtags,getSurgeBitmap(IDB_FILTERSUBTYPE));
					rect(1,1,9,14);
					((gui_switch*)hsw)->is_itype = true;
					((gui_switch*)hsw)->imax = 3;
					((gui_switch*)hsw)->ivalue = p->val.i+1;
					if(fut_subcount[synth->storage.getPatch().scene[current_scene].filterunit[p->ctrlgroup_entry].type.val.i] == 0) ((gui_switch*)hsw)->ivalue = 0;					
					rect.offset(p->posx+129,p->posy+1);
					hsw->setMouseableArea(rect);
					hsw->setValue(p->get_value_f01());
					frame->addView(hsw);
			...
			case ct_bool_keytrack:
				{
					CRect rect(0,0,43,7);
					rect.offset(p->posx,p->posy);
					CControl *hsw = new gui_switch(rect,this,p->id+start_paramtags,getSurgeBitmap(IDB_SWITCH_KTRK));
					hsw->setValue(p->get_value_f01());
					frame->addView(hsw);
				}
				break;
			case ct_bool_retrigger:
				{
					CRect rect(0,0,43,7);
					rect.offset(p->posx,p->posy);
					CControl *hsw = new gui_switch(rect,this,p->id+start_paramtags,getSurgeBitmap(IDB_SWITCH_RETRIGGER));
					hsw->setValue(p->get_value_f01());
					frame->addView(hsw);
				}
				break;	
			case ct_oscroute:
				{
					...
					rect.offset(p->posx,p->posy);
					CControl *hsw = new gui_hswitch(rect,this,p->id+start_paramtags,3,15,1,3,getSurgeBitmap(IDB_OSCROUTE),nopoint,true);
					hsw->setValue(p->get_value_f01());
					frame->addView(hsw);
				}
				break;
			case ct_envshape:
            {
               bool hasShape = synth->storage.getPatch().scene[current_scene].adsr[p->ctrlgroup_entry].mode.val.i == emt_digital;

               if (hasShape)
               {
                  CRect rect(0, 0, 20, 14);
                  rect.offset(p->posx, p->posy);
                  gui_hswitch *hsw = new gui_hswitch(rect, this, p->id + start_paramtags, 3, 14, 1, 3, getSurgeBitmap(IDB_ENVSHAPE), nopoint, true);
                  hsw->setValue(p->get_value_f01());
                  if (p->name[0] == 'd') hsw->imgoffset = 3;
                  else if (p->name[0] == 'r') hsw->imgoffset = 6;
         ...
         case ct_envmode:
            {
					...
					rect.offset(p->posx,p->posy);
					gui_hswitch *hsw = new gui_hswitch(rect,this,p->id+start_paramtags,2,15,2,1,getSurgeBitmap(IDB_ENVMODE),nopoint,false);
					hsw->setValue(p->get_value_f01());
			...
			case ct_lfotrigmode:
				{
					...
					rect.offset(p->posx,p->posy);
					CControl *hsw = new gui_hswitch(rect,this,p->id+start_paramtags,3,39,3,1,getSurgeBitmap(IDB_LFOTRIGGER),nopoint,true);
					hsw->setValue(p->get_value_f01());
					frame->addView(hsw);
				}
				break;				
			case ct_bool_mute:
				{
					CRect rect(0,0,22,15);
					rect.offset(p->posx,p->posy);
					CControl *hsw = new gui_switch(rect,this,p->id+start_paramtags,getSurgeBitmap(IDB_SWITCH_MUTE));
					hsw->setValue(p->get_value_f01());
					frame->addView(hsw);
				}
				break;
			case ct_bool_solo:
				{
					CRect rect(0,0,22,15);
					rect.offset(p->posx,p->posy);
					CControl *hsw = new gui_switch(rect,this,p->id+start_paramtags,getSurgeBitmap(IDB_SWITCH_SOLO));
					hsw->setValue(p->get_value_f01());
					frame->addView(hsw);
				}
				break;
			case ct_bool_unipolar:
				{
					...
					rect.offset(p->posx,p->posy);
					CControl *hsw = new gui_switch(rect,this,p->id+start_paramtags,getSurgeBitmap(IDB_UNIPOLAR));
					hsw->setValue(p->get_value_f01());
					frame->addView(hsw);
				}
				break;				
			case ct_bool_relative_switch:
				{
					...
					CControl *hsw = new gui_switch(rect,this,p->id+start_paramtags,getSurgeBitmap(IDB_RELATIVE_TOGGLE));
					rect(1,1,9,14);
					rect.offset(p->posx+129,p->posy+5);
					hsw->setMouseableArea(rect);
					hsw->setValue(p->get_value_f01());
			...
			case ct_bool_link_switch:
				{
					CRect rect(0,0,12,18);
					rect.offset(p->posx+129,p->posy+5);
					CControl *hsw = new gui_switch(rect,this,p->id+start_paramtags,getSurgeBitmap(IDB_SWITCH_LINK));					
					rect(1,1,9,14);
					rect.offset(p->posx+129,p->posy+5);
					hsw->setMouseableArea(rect);
					hsw->setValue(p->get_value_f01());
			...
			case ct_osctype:
				{	
					...
					CControl *hsw = new COscMenu(rect,this,tag_osc_menu,&synth->storage,&synth->storage.getPatch().scene[current_scene].osc[current_osc]);
					hsw->setValue(p->get_value_f01());
					frame->addView(hsw);
				}
				break;
			case ct_fxtype:
				{
					...
					rect.offset(p->posx,p->posy);
					//CControl *m = new CFxMenu(rect,this,tag_fx_menu,&synth->storage,&synth->storage.getPatch().fx[current_fx],current_fx);
					CControl *m = new CFxMenu(rect,this,tag_fx_menu,&synth->storage,&synth->storage.getPatch().fx[current_fx],&synth->fxsync[current_fx],current_fx);					
			...
			case ct_wstype:
				{
					...
					rect.offset(p->posx,p->posy);
					CControl *hsw = new gui_hswitch(rect,this,p->id+start_paramtags,6,47,6,1,getSurgeBitmap(IDB_WAVESHAPER),nopoint,true);									
					rect(0,0,28,47);
					rect.offset(p->posx,p->posy);
					hsw->setMouseableArea(rect);
					hsw->setValue(p->get_value_f01());
			...
			case ct_polymode:
				{
					...
					rect.offset(p->posx,p->posy);
					CControl *hsw = new gui_hswitch(rect,this,p->id+start_paramtags,6,47,6,1,getSurgeBitmap(IDB_POLYMODE),nopoint,true);									
					rect(0,0,50,47);
					rect.offset(p->posx,p->posy);
					hsw->setMouseableArea(rect);
					hsw->setValue(p->get_value_f01());
			...
			case ct_fxbypass:
				{
					...
					rect.offset(p->posx,p->posy);
					CControl *hsw = new gui_hswitch(rect,this,p->id+start_paramtags,4,27,1,4,getSurgeBitmap(IDB_FXBYPASS),nopoint,true);
			...
			case ct_pitch_octave:
				{
					...
					CControl *hsw = new gui_hswitch(rect,this,p->id+start_paramtags,7,18,1,7,getSurgeBitmap(IDB_OCTAVES),nopoint,true);									
					rect(1,0,91,14);
					rect.offset(p->posx,p->posy);
					hsw->setMouseableArea(rect);
					hsw->setValue(p->get_value_f01());
			...
			case ct_fbconfig:
				{
					...
					rect.offset(p->posx,p->posy);
					CControl *hsw = new gui_hswitch(rect,this,p->id+start_paramtags,8,52,1,8,getSurgeBitmap(IDB_FBCONFIG),nopoint,true);														
					hsw->setValue(p->get_value_f01());
					frame->addView(hsw);
			...
			case ct_fmconfig:
				{
					CRect rect(0,0,134,52);
					rect.offset(p->posx,p->posy);
					CControl *hsw = new gui_hswitch(rect,this,p->id+start_paramtags,4,52,1,4,getSurgeBitmap(IDB_FMCONFIG),nopoint,true);
					hsw->setValue(p->get_value_f01());
					frame->addView(hsw);
				}
				break;
			case ct_scenemode:
				{
					...
					rect.offset(p->posx,p->posy);
					CControl *hsw = new gui_hswitch(rect,this,p->id+start_paramtags,3,27,3,1,getSurgeBitmap(IDB_SCENEMODE),nopoint,true);														
					rect(1,1,35,27);
					rect.offset(p->posx,p->posy);
					hsw->setMouseableArea(rect);
					hsw->setValue(p->get_value_f01());
			...
			case ct_lfoshape:
				{
					...
					{
						CControl *slfo = new gui_superlfo(rect,lfo_id == 0, this,p->id+start_paramtags,&synth->storage.getPatch().scene[current_scene].lfo[lfo_id],&synth->storage,&synth->storage.getPatch().stepsequences[current_scene][lfo_id]);
						lfodisplay = slfo;
						frame->addView(slfo);
					}
				}
			...
			case ct_scenesel:
				{
					...
					rect.offset(p->posx,p->posy);
					CControl *sceneswitch = new gui_hswitch(rect,this,tag_scene_select,2,27,1,2,getSurgeBitmap(IDB_SCENESWITCH),nopoint);									
			...
			case ct_character:
				{					
					...
					rect.offset(p->posx,p->posy);
					CControl *hsw = new gui_hswitch(rect,this,p->id+start_paramtags,3,12,1,3,getSurgeBitmap(IDB_CHARACTER),nopoint,true);
					hsw->setValue(p->get_value_f01());
					frame->addView(hsw);
				}
				break;
			...
			default:
				{					
					...
					{
						gui_slider *hs = new gui_slider(CPoint(p->posx,p->posy + p->posy_offset*yofs),style,this,p->id+start_paramtags,true);
					...
					{
						gui_slider *hs = new gui_slider(CPoint(p->posx,p->posy + p->posy_offset*yofs),style,this,p->id+start_paramtags);
						hs->setValue(p->get_value_f01());
						...
						/*						if(p->can_temposync() && (style & kHorizontal))
						{
						CRect rect(0,0,14,18);
						rect.offset(p->posx+134,p->posy+5 + p->posy_offset*yofs);
						CControl *hsw = new gui_switch(rect,this,p->id+start_paramtags+tag_temposyncoffset,bmp_temposync);
						rect(1,1,11,14);
						rect.offset(p->posx+134,p->posy+5 + p->posy_offset*yofs);
						hsw->setMouseableArea(rect);
	...
	if(synth->storage.getPatch().scene[current_scene].f2_link_resonance.val.b)
	{
		...
		if(param[i] && dynamic_cast<gui_slider*>(param[i]) != nullptr) ((gui_slider*)param[i])->disabled = true;		
	}	

	// pan2 control
	if((synth->storage.getPatch().scene[current_scene].filterblock_configuration.val.i != fb_stereo)&&(synth->storage.getPatch().scene[current_scene].filterblock_configuration.val.i != fb_wide))
	{
	...
	gui_hswitch *b_about = new gui_hswitch(aboutbrect,this,tag_about,1,27,1,1,getSurgeBitmap(IDB_BUTTON_ABOUT),nopoint,false);
   ...
   gui_hswitch *cancelButton = new gui_hswitch(CRect(CPoint(305, 147), CPoint (62, 25)), this, tag_store_cancel, 1, 25, 1, 1, nullptr, nopoint, false);
   saveDialog->addView(cancelButton);
   gui_hswitch *okButton = new gui_hswitch(CRect(CPoint(373, 147), CPoint(62, 25)), this, tag_store_ok, 1, 25, 1, 1, nullptr, nopoint, false);
...
void sub3_editor::setParameter (long index, float value)
{
	...
	if (index>synth->storage.getPatch().param_ptr.size()) return;
	...
	//if(param[index])
	{
		...
		while(j<7)
		{
			if ((synth->refresh_ctrl_queue[j] > -1) && (synth->refresh_ctrl_queue[j] != index)) j++;	
			else break;
		}
...
{ 
	...
	if(button&kDoubleClick) button |= kControl;
	...
	if((tag >= tag_mod_source0)&&(tag < tag_mod_source_end))
	{
		...
		if(button&kRButton)
		{
			...
			frame->localToFrame(where);
			menuRect.offset (where.x, where.y);
			COptionMenu *contextMenu = new COptionMenu(menuRect, 0, 0, 0, 0, kNoDrawStyle|kMultipleCheckStyle);
			int eid = 0;
			int id_clearallmr=-1,id_learnctrl=-1,id_clearctrl=-1,id_bipolar=-1,id_copy=-1,id_paste=-1,id_rename=-1;
			...
			int clear_md[max_md];			
			for(int md=0; md<n_total_md; md++) clear_md[md] = -1;
			...
			{
				if(((md<n_global_params)||((synth->storage.getPatch().param_ptr[md]->scene-1) == synth->storage.getPatch().scene_active.val.i)) && synth->isActiveModulation(md,modsource)) 
				{
					...
					char tmptxt[256];						
					sprintf(tmptxt,"Clear %s -> %s [%.2f]",(char*)modsource_abberations[modsource],synth->storage.getPatch().param_ptr[md]->get_full_name(),synth->getModDepth(md,modsource));					
					clear_md[md] = eid;
			...
			{				
				...
				if(synth->learn_custom > -1) cancellearn = true;
				id_learnctrl = eid;
				if (cancellearn) contextMenu->addEntry("Abort learn controller",eid++);
				else contextMenu->addEntry("Learn controller [MIDI]",eid++);								
				...
				contextMenu->checkEntry (id_bipolar, synth->storage.getPatch().scene[0].modsources[ms_ctrl1+ccid]->is_bipolar());
				id_rename = eid;
			...
			if(command>=0)
			{
				...
				{
					for(int md=1; md<n_total_md; md++) synth->clearModulation(md,modsource);
					refresh_mod();
				} 
				else if (command == id_learnctrl)
				{
					if (cancellearn) synth->learn_custom = -1;
					else synth->learn_custom = ccid;
				}
				...
				{					
					bool bp = !synth->storage.getPatch().scene[0].modsources[ms_ctrl1+ccid]->is_bipolar();
					synth->storage.getPatch().scene[0].modsources[ms_ctrl1+ccid]->set_bipolar(bp);

					float f = synth->storage.getPatch().scene[0].modsources[ms_ctrl1+ccid]->get_output01();
					control->setValue(f);
				...
				{					
					if(lfo_id >= 0) synth->storage.clipboard_copy(cp_lfo, sc, lfo_id);
				}
				else if (command == id_paste)
				{					
					if(lfo_id >= 0) synth->storage.clipboard_paste(cp_lfo, sc, lfo_id);
					queue_refresh = true;
				}
				else if(command == id_rename)
				{						
					spawn_miniedit_text(synth->storage.getPatch().CustomControllerLabel[ccid],16);
					((gui_modsrcbutton*)control)->setlabel(synth->storage.getPatch().CustomControllerLabel[ccid]);
	...
	if (tag<start_paramtags) return 0; 
	if (!(button & (kControl|kRButton))) return 0;
	...
	{	
		...
		if((button&kRButton)&&(p->valtype == vt_float))
		{
			...
			COptionMenu *contextMenu = new COptionMenu(menuRect, 0, 0, 0, 0, kNoDrawStyle|kMultipleCheckStyle);
			int eid=0;
			int id_temposync=-1,id_clearallmr=-1,id_extendrange=-1, id_learnctrl=-1, id_clearctrl=-1, id_absolute=-1;
			...
			{
				id_learnctrl = eid;
				if (synth->learn_param > -1) cancellearn = true;
				if (cancellearn) contextMenu->addEntry("Abort learn controller",eid++);				
				else contextMenu->addEntry("Learn controller [midi]",eid++);				
			}
			...
			for(int ms=1; ms<n_modsources; ms++) if(synth->isActiveModulation(ptag,ms)) n_ms++;			
			...
			{		
				...
				for(int ms=1; ms<n_modsources; ms++)
				{
					...
					{
						char tmptxt[256];						
						sprintf(tmptxt,"Clear %s -> %s [%.2f]",(char*)modsource_abberations[ms],p->get_name(),synth->getModDepth(ptag,ms));
						clear_ms[ms] = eid;
						contextMenu->addEntry(tmptxt,eid++);						
					}
					else clear_ms[ms] = -1;
			...
			if(command>=0)
			{
				...
				{					
					if (cancellearn) synth->learn_param = -1;
					else synth->learn_param = p->id;
				}
				else if (command == id_clearctrl)
				{
					...
					{
						int a = ptag;
						if (ptag >= (n_global_params+n_scene_params)) a -= ptag;
		...
		else if(button&kControl)
		{
			...
			else
			{
				...
				if (oscdisplay && (p->ctrlgroup == 2)) oscdisplay->setDirty();
				if (lfodisplay && (p->ctrlgroup == 6)) lfodisplay->setDirty();
				return 0;		
			}
		}
		else return 0;
...
{		
	...
	if((tag >= tag_mod_source0)&&(tag < tag_mod_source_end))
	{
		...
		{	
			...
			((modulation_controller*)synth->storage.getPatch().scene[0].modsources[t])->set_target01(control->getValue(),false);
	...
	if((tag == f1subtypetag)||(tag == f2subtypetag))
	{
		...
		int nn = fut_subcount[synth->storage.getPatch().scene[current_scene].filterunit[idx].type.val.i];
		if(a >= nn) a = 0;
		synth->storage.getPatch().scene[current_scene].filterunit[idx].subtype.val.i = a;
		if(!nn) ((gui_switch*)control)->ivalue = 0;
		else ((gui_switch*)control)->ivalue = a+1;
		control->invalid();
	...
	case tag_mp_category:
		{
			if (control->getValue() > 0.5f) synth->increment_patch(1,0);
			else synth->increment_patch(-1,0);
			return;
		}
		break;
	case tag_mp_patch:
		{
			if (control->getValue() > 0.5f) synth->increment_patch(0,1);
			else synth->increment_patch(0,-1);
			return;
		}
		break;
	case tag_osc_select:
	...
	case tag_about:
		{
			if(aboutbox) ((CAboutBox*)aboutbox)->boxShow();
		}
		break;
	case tag_store:
		{
					...
					CFUserNotificationDisplayAlert(0,kCFUserNotificationPlainAlertLevel,0,0,0,CFSTR("Demo limitation"),
					CFSTR("Saving patches isn't supported in the demo version of Surge. (including sequencer recall)"),CFSTR("Ok"),0,0,&responseFlags);
				#else
					MessageBox(::GetActiveWindow(),L"Saving patches isn't supported in the demo version of Surge. (including sequencer recall)",L"Demo limitation",MB_OK | MB_ICONWARNING);		
				#endif
			#else
			patchdata p;
			...
			if(p.author.empty()) p.author = synth->storage.defaultname;
			if(p.comments.empty()) p.comments = synth->storage.defaultsig;
         ...
         showPatchStoreDialog(&p, &synth->storage.patch_category, synth->storage.patch_category_split[1]);
			#endif			
		}
		break;
   ...
   break;
	default:
		{
			...
			{	
				...
				//if(modsource && mod_active && (p->valtype == (valtypes)vt_float) && !((p->ctrlgroup == 6)&&(p->ctrlgroup_entry >= ms_lfo1)&&!can_modulate_modulators(modsource)) && (p->modulateable))
				if(modsource && mod_editor && synth->isValidModulation(p->id,modsource) && dynamic_cast<gui_slider*>(control) != nullptr)
				{
				...
				else
				{
					...
					{										
						...
						if (dynamic_cast<gui_slider*>(control) != nullptr) ((gui_slider*)control)->SetQuantitizedDispValue(p->get_value_f01());
						...
						if(p->ctrltype == ct_polymode) modulate = true;														
					}
				}
				if(!queue_refresh)
				{
					...
					if (oscdisplay && 
						((p->ctrlgroup == 2) || (p->ctrltype == ct_character)))
					{
						oscdisplay->setDirty();
						oscdisplay->invalid();
	...
	if((tag == (f1subtypetag-1))||(tag == (f2subtypetag-1)))
	{
		...
		if(a >= nn) a = 0;
		synth->storage.getPatch().scene[current_scene].filterunit[idx].subtype.val.i = a;
		if(!nn) ((gui_switch*)filtersubtype[idx])->ivalue = 0;
		else ((gui_switch*)filtersubtype[idx])->ivalue = a+1;
	...
	if(tag == filterblock_tag)
	{
		...
		if(param[i] && dynamic_cast<gui_slider*>(param[i]) != nullptr) ((gui_slider*)param[i])->disabled = (synth->storage.getPatch().scene[current_scene].filterblock_configuration.val.i != fb_stereo)&&(synth->storage.getPatch().scene[current_scene].filterblock_configuration.val.i != fb_wide);
		param[i]->setDirty();
	}
	if(tag == fxbypass_tag)	// still do the normal operation, that's why it's outside the switch-statement
	{
		if(ccfxconf) ((gui_fxconf*)ccfxconf)->set_bypass(synth->storage.getPatch().fx_bypass.val.i);
...
void sub3_editor::draw_infowindow(int ptag, CControl *control, bool modulate, bool forceMB)
{
	...
	if(buttons && forceMB) return;		// don't draw via CC if MB is down

	CRect r(0,0,148,18);
	if (modulate) r.bottom += 18;
   CRect r2 = control->getViewSize();

	r.offset((r2.left / 150) * 150, r2.bottom);
	
	int ao = 0;
	if (r.bottom > window_size_y) ao = (window_size_y - r.bottom);
	r.offset(0,ao);		
...
bool sub3_editor::showPatchStoreDialog(patchdata * p, vector<patchlist_category>* patch_category, int startcategory)
{
