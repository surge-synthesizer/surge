...
gui_slider::gui_slider(const CPoint &loc, long stylee, IControlListener *listener, long tag, bool is_mod)
...
{		
   ...
   CRect size;
	...
	{
		if(!(style & kTop)) style |= kBottom;		// kBottom by default

		pTray = getSurgeBitmap(IDB_FADERV_BG);
		pHandle = getSurgeBitmap(IDB_FADERV_HANDLE);		

		if(style & kWhite) typehy = 0;
		else typehy = 1;		

		if (style & kMini) range = 39;
		else range = 56;	
...
{			
   ...
   CRect size = getViewSize();
	...
	if (style & kHorizontal)
	{
		if(style & kSemitone) typey = 2;
		else if(style & kBipolar) typey = 1;			
		else typey = 0;
		if(style & kWhite) typey += 3;		 			
	}
	else
	{
		if(style & kMini) typey = 2;
		else if (style & kBipolar) typey = 1;
		else typey = 0;
	}
	...
	if (has_modulation_current) typex = 2;

	if (pTray)
	{
		//CRect trect(0,0,pTray->getWidth(),pTray->getHeight());
		CRect trect;
		if(style&kHorizontal) trect = CRect(0,0,133,14);
		else trect = CRect(0,0,16,75);
		
		trect.offset(size.left,size.top);

		if(style&kHorizontal) trect.offset(2,5);
		else trect.offset(2,2);
		...
		if(style&kHorizontal) pTray->draw(dc,trect,CPoint(133*typex,14*typey),alpha);
		else pTray->draw(dc,trect,CPoint(16*typex,75*typey),alpha);
	}
	if(disabled) return;

	CRect headrect;
	if(style&kHorizontal) headrect = CRect(0,0,28,24);
	else headrect = CRect(0,0,24,28);
	...
	{		
		...
		if(style & kWhite) dc->setFontColor(kWhiteCColor);
		else	dc->setFontColor(kBlackCColor);		
...
//		label[1] = a;
		...
		if(leftlabel[0]) dc->drawString(leftlabel,trect,kLeftText, true);
	...
	if (pHandle && (modmode != 2))
	{
		...
		if(style&kHorizontal) hrect.offset(0,3);		

		float dispv = limit_range(qdvalue,0.f,1.f);
		if (style & kRight || style & kBottom) dispv = 1-dispv;
		dispv *= range;
		...
		if(style&kHorizontal) pHandle->draw(dc,hrect,CPoint(0,24*typehy),modmode?0x7f:0xff);
		else pHandle->draw(dc,hrect,CPoint(0,28*typehy),modmode?0x7f:0xff);
	...
	if(pHandle && modmode)
	{
		...
		float dispv;
		if (modmode == 2) dispv = limit_range(0.5f + 0.5f*modval,0.f,1.f);
		else dispv = limit_range(modval + value,0.f,1.f);
		...
		if(style&kHorizontal) pHandle->draw(dc,hrect,CPoint(28,24*typehy),0xff);
		else pHandle->draw(dc,hrect,CPoint(24,28*typehy),0xff);
	}
...
CMouseEventResult gui_slider::onMouseDown (CPoint &where, const CButtonState& buttons)
{
   CCursorHidingControl::onMouseDown(where, buttons);
	if(disabled) return kMouseDownEventHandledButDontNeedMovedOrUpEvents;
	if(controlstate)
	{		
#if MAC
...
CMouseEventResult gui_slider::onMouseUp (CPoint &where, const CButtonState& buttons)
{
   CCursorHidingControl::onMouseUp(where, buttons);
	if(disabled) return kMouseEventHandled;
	if(controlstate)
	{
#if MAC
		/*if(buttons & kRButton)
...
double gui_slider::getMouseDeltaScaling(CPoint & where, const CButtonState & buttons)
{
   ...
   if (buttons & kRButton) rate *= 0.1;
   if (buttons & kShift) rate *= 0.1;
...
void gui_slider::onMouseMoveDelta(CPoint &where, const CButtonState& buttons, double dx, double dy)
{
   if (disabled) return;
	...
	{
      if (!edit_value) return;
		CPoint p;

		double diff;
		if (style & kHorizontal) diff = dx;
		else diff = dy;

		if (style & kRight || style & kBottom) diff = -diff;
