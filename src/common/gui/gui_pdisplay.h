...
class gui_pdisplay : public CControl
{
public:
	gui_pdisplay (const CRect &size) 
	: CControl(size,0,0,0)
	{
		label[0][0] = 0;
		label[1][0] = 0;
	...
	{
		if (txt1) strncpy(label[0],txt1,256);
		else label[0][0] = 0;		
		if (txt2) strncpy(label[1],txt2,256);
		else label[1][0] = 0;
		setDirty (true);
	}
	...
	virtual void draw (CDrawContext*dc)
	{
		if(visible)
		{			
			//COffscreenContext *dc = COffscreenContext::create(getFrame(),size.width(),size.height());
         ...
         sizem1.inset(1, 1);
			...
			if(label[0][0])dc->drawString(label[0],tupper,kLeftText, true);
...
protected:
	...
	CLASS_METHODS(gui_pdisplay, CControl)
};