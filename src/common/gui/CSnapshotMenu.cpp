...
CSnapshotMenu::CSnapshotMenu(const CRect &size, IControlListener *listener, long tag, sub3_storage *storage)
...
void CSnapshotMenu::populate()
{
   ...
   if (sect)
   {
      ...
      while (type)
      {
         int type_id = 0; type->Attribute("i", &type_id);
         sub = 0;
      ...
      if ((sel_id >= 0) && (sel_id < contextMenu->getNbEntries()))
      {
         if (sel_id < main)
         {
            int type_id = 0; xpp[sel_id&(max_main - 1)]->Attribute("i", &type_id);		// get "i" value from xmldata
            load_snapshot(type_id, 0);
         }
         else
         {
            char name[namechars];
      ...
      else if (b && within_range(0, main_id, max_main) && within_range(0, sub_id, max_sub))
      {
         ...
         {
            int type_id = 0; xpp[main_id&(max_main - 1)]->Attribute("i", &type_id);		// get "i" value from xmldata
...
COscMenu::COscMenu(const CRect &size, IControlListener *listener, long tag, sub3_storage *storage, sub3_osc *osc)
...
{		
	...
	if(e)
	{
		for(int i=0; i<n_osc_params; i++)
		{
			...
			{
				if(e->QueryDoubleAttribute(lbl,&d) == TIXML_SUCCESS) osc->p[i].val.f = (float)d;
			}
			else
			{
				if(e->QueryIntAttribute(lbl,&j) == TIXML_SUCCESS) osc->p[i].val.i = j;
			}
		}
	}
}*/
...
CFxMenu::CFxMenu(const CRect &size, IControlListener *listener, long tag, sub3_storage *storage, sub3_fx *fx, sub3_fx *fxbuffer, int slot)
: CSnapshotMenu(size,listener,tag,storage)
{
	strcpy(mtype,"fx");
	this->fx = fx;
...
void CFxMenu::draw (CDrawContext*dc)
{
   ...
   txtbox.bottom += 2;
	...
	dc->drawPoint(d,kBlackCColor); d.x++;
	dc->drawPoint(d,kBlackCColor); d.y++;
	dc->drawPoint(d,kBlackCColor); d.y--; d.x++;
...
{				
	if(!type) fxbuffer->type.val.i = type;
	if(e)
	{
		...
		for(int i=0; i<n_fx_params; i++)
		{
			...
			{
				if(e->QueryDoubleAttribute(lbl,&d) == TIXML_SUCCESS) fxbuffer->p[i].set_storage_value((float)d);
			}
			else
			{
				if(e->QueryIntAttribute(lbl,&j) == TIXML_SUCCESS) fxbuffer->p[i].set_storage_value(j);
			}

			sprintf(sublbl,"p%i_temposync",i);
			fxbuffer->p[i].temposync = ((e->QueryIntAttribute(sublbl,&j) == TIXML_SUCCESS) && (j==1));
			sprintf(sublbl,"p%i_extend_range",i);
			fxbuffer->p[i].extend_range = ((e->QueryIntAttribute(sublbl,&j) == TIXML_SUCCESS) && (j==1));
		}
	}
}
void CFxMenu::saveSnapshot(TiXmlElement *e, const char *name)
{		
	if(fx->type.val.i == 0) return;
