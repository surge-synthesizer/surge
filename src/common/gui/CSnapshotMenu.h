...
{
public:
	COscMenu(const CRect &size, IControlListener *listener, long tag, sub3_storage *storage, sub3_osc *osc);
	virtual void draw (CDrawContext*dc);
	virtual void loadSnapshot(int type, TiXmlElement *e);
protected:	
...
{
public:
	CFxMenu(const CRect &size, IControlListener *listener, long tag, sub3_storage *storage, sub3_fx *fx, sub3_fx *fxbuffer, int slot);
	virtual void draw (CDrawContext*dc);
