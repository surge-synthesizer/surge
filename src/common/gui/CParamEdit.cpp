...
CParamEdit::CParamEdit(const CRect &size, IControlListener *listener, long tag, CBitmap *pBackground) : CControl(size,listener,tag,pBackground)
{		
...
void CParamEdit::setControlMode(int mode)
{ 
	controlmode = mode;
...
//------------------------------------------------------------------------
void CParamEdit::setValue(float val)
{
...
//------------------------------------------------------------------------
void CParamEdit::draw (CDrawContext *pContext)
{	
	//COffscreenContext *tempContext = new COffscreenContext(this->getFrame(),drawsize.width(),drawsize.height());
	...
	if(altlook) pContext->setFillColor(orange);
	else 
	{
	...
	case cm_noyes:
		if(i_value) sprintf(the_text,"yes");		
		else sprintf(the_text,"no");
		break;
	case cm_none:
		sprintf(the_text,"---");
		break;
	...
	if(altlook) pContext->setFontColor(kWhiteCColor);
	else pContext->setFontColor(fontColor);		
		
   CRect labelSize(sze);
   labelSize.top++;
   pContext->drawString(the_text, labelSize,kCenterText, true);
	CRect cpysize(drawsize);
	cpysize.right++;	cpysize.bottom++;
...
//------------------------------------------------------------------------
void CParamEdit::bounceValue()
{	
	if (i_value>i_max) i_value = i_max;
	if (i_value<i_min) i_value = i_min;
...
CMouseEventResult CParamEdit::onMouseDown (CPoint &where, const CButtonState& buttons)
{
	if (controlmode == cm_int_menu) return kMouseDownEventHandledButDontNeedMovedOrUpEvents;
...
CMouseEventResult CParamEdit::onMouseUp (CPoint &where, const CButtonState& buttons)
{	
	if(controlstate)
	{		
		endEdit();
...
CMouseEventResult CParamEdit::onMouseMoved (CPoint &where, const CButtonState& buttons)
{
	if((controlstate == cs_drag) && (buttons & kLButton))
	{
		...
		if(buttons & kShift) delta *= 0.1;
		if(buttons & kRButton) delta *= 0.1;
...
//------------------------------------------------------------------------
void CParamEdit::setFontColor (CColor color)
{
	// to force the redraw
	if (fontColor != color)
		setDirty ();
...
//------------------------------------------------------------------------
void CParamEdit::setBackColor (CColor color)
{
	// to force the redraw
	if (backColor != color)
		setDirty ();
...
//------------------------------------------------------------------------
void CParamEdit::setLineColor (CColor color)
{
	// to force the redraw
	if (lineColor != color)
		setDirty ();
...
void CParamEdit::setLabelPlacement(int placement)
{ 
	...
	if(label[0])
	{
		if(labelplacement == lp_left)	newsize.left -= width + margin;
		if(labelplacement == lp_right)	newsize.right += width + margin;
		if(labelplacement == lp_above)	newsize.top -= height + vmargin;
		if(labelplacement == lp_below)	newsize.bottom += height + vmargin;
	}
	
	setViewSize(newsize);
}

void CParamEdit::setLabel(char *newlabel)
{
