...
class gui_slider : public CCursorHidingControl
{
public:
	gui_slider(const CPoint &loc, long style, IControlListener *listener = 0, long tag = 0, bool is_mod=false);
	~gui_slider();
   ...
   virtual void onMouseMoveDelta(CPoint &where, const CButtonState& buttons, double dx, double dy);
	...
	virtual void setModMode(int mode){ modmode = mode; }	// 0 - absolute pos, 1 - absolute mod, 2 - relative mod
	virtual void setMoveRate(float rate){ moverate = rate; }
	virtual void setModPresent(bool b){ has_modulation = b;}			// true if the slider has modulation routings assigned to it	
	virtual void setModCurrent(bool b){ has_modulation_current = b; invalid();}	// - " " - for the currently selected modsource
   ...
   virtual bool isInMouseInteraction();
	...
	CLASS_METHODS(gui_slider,CControl)
