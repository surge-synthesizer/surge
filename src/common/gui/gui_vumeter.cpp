...
gui_vumeter::gui_vumeter (const CRect &size) 
: CControl(size,0,0,0)
{
	stereo = true;
	valueR = 0.0;
...
void gui_vumeter::setType(int vutype)
{		
	type = vutype;
	switch(type)
	{
...
void gui_vumeter::setValueR(float f)
{
   if (f != valueR)
   {
      valueR = f;
...
void gui_vumeter::draw (CDrawContext*dc)
{	
   ...
   CRect size = getViewSize();
	...
	f1.inset(1,1); f2.inset(0,2);
	dc->drawRect(f1); dc->drawRect(f2);
	...
	{	
      ...
      bar.top++;
		...
		if(value > 1.0f) dc->setFillColor(kRedCColor);
		dc->drawRect(bar, kDrawFilled);
		...
		if(stereo)
		{
         ...
         midline.top++;
			...
			bar.offset(0,3);	barblack.offset(0,3);
			bar.right = bar.left + (scale(valueR)*w);
			dc->setFillColor(vugreen);
			if(valueR > 1.0f) dc->setFillColor(kRedCColor);			
			dc->drawRect(bar, kDrawFilled);
