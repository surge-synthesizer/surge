...
void gui_wavedisplay::draw (CDrawContext *dc)
{			
	pdata tp[n_scene_params];	
	tp[oscdata->pitch.param_id_in_scene].f = 0;	
	for(int i=0; i<n_osc_params; i++) tp[oscdata->p[i].param_id_in_scene].i = oscdata->p[i].val.i;
   ...
   cdisurf->begin();
	...
	int h = h2;
	assert(h<512);		
	if(uses_wavetabledata(oscdata->type.val.i)) h -= wtbheight;
	...
	if(osc)
	{
		...
		if (use_display) osc->init(disp_pitch_rs,true);
		...
		for(int x=0; x<getWidth(); x++)
		{
			...
			for(int s = 0; s<aa_samples; s++)
			{
				...
				float val = 0.f;
				if(use_display) val = -osc->output[block_pos];								
   ...
   if (uses_wavetabledata(oscdata->type.val.i))
   {
      ...
      dc->setFontColor(kBlackCColor);
      if(oscdata->wt.flags & wtf_is_sample) dc->drawString("IS SAMPLE",wtlbl_status,false,kRightText);*/
...
bool gui_wavedisplay::onDrop (IDataPackage* drag, const CPoint& where)
{
	/*long size,type;
...
CMouseEventResult gui_wavedisplay::onMouseDown (CPoint &where, const CButtonState& button)
{	
	if (!(button & kLButton))
	...
	else if(uses_wavetabledata(oscdata->type.val.i))
	{
		...
		if(diff)
		{
			oscdata->wt.queue_id = (oscdata->wt.current_id + diff + storage->wt_list.size()) % storage->wt_list.size();							
		} 
		else if(rmenu.pointInside(where))	
		{
			...
			for(int c = 0; c < storage->wt_category.size(); c++)
			{
				...
				for(p = 0; p < storage->wt_list.size(); p++)
				{
                  ...
                  auto action = [this, p](CCommandMenuItem* item)
                  {
                     this->loadWavetable(p);
                  };
...
void gui_wavedisplay::loadWavetable(int id)
{
...
CMouseEventResult gui_wavedisplay::onMouseUp (CPoint &where, const CButtonState& buttons)
{
	if(controlstate)
	{
		controlstate = 0;
...
CMouseEventResult gui_wavedisplay::onMouseMoved (CPoint &where, const CButtonState& buttons)
{
	if(controlstate)
	{		
