...
void CLFOGui::draw(CDrawContext *dc)
{
    ...
    else
    {
        ...
        dc->setLineWidth(1.0);
        // LFO ruler bounds AKA the upper and lower horizontal lines that define the bounds that the
        // waveform draws in
        ...
        if (drawBeats)
        {
            for (auto l = 0; l < nBeats; ++l)
            {
                ...
                if (l % (tsNum * everyMeasure) == 0)
                {
                    ...
                    // dc->drawLine(mps,mp); // this draws the hat on the bar which I decided to
                    // skip
    ...
    if (typeImg)
    {
        ...
        typeImg->draw(dc, CRect(CPoint(leftpanel.left, leftpanel.top + 2), CPoint(51, 76)),
                      CPoint(0, off), 0xff);
        ...
        if (lfo_type_hover >= 0)
        {
            ...
            if (lfo_type_hover == type)
            {
                ...
                {
                    typeImgHoverOn->draw(
                        dc, CRect(CPoint(leftpanel.left, leftpanel.top + 2), CPoint(51, 76)),
                        CPoint(0, off), 0xff);
                }
            }
            else if (typeImgHover)
            {
                typeImgHover->draw(dc,
                                   CRect(CPoint(leftpanel.left, leftpanel.top + 2), CPoint(51, 76)),
                                   CPoint(0, off), 0xff);
            }
        }
    }
    else
    {
        for (int i = 0; i < n_lfo_types; i++)
        {
            ...
            tb.bottom = tb.top + 10;
            // std::cout << std::hex << this << std::dec << " CHECK " << i << " " <<
            // lfodata->shape.val.i;
                          ...
                          VSTGUI::CRect &leftpanel)
{
    ...
    auto shiftTranslate =
        CGraphicsTransform().translate(size.left, size.top).translate(splitpoint, 0);
    {
        CDrawContext::Transform shiftTranslatetransform(*dc, shiftTranslate);
        for (int i = 0; i < n_stepseqsteps; i++)
        {
            CRect rstep(1.f * i * scale, 1.f * margin, 1.f * (i + 1) * scale - 1,
                        1.f * h - margin2 + 1),
                gstep;
            ...
            if (edit_trigmask)
            {
                ...
                if (controlstate == cs_trigtray_toggle &&
                    (i == selectedSSrow || draggedIntoTrigTray[i]))
                {
    ...
    */
    ...
    if (controlstate == cs_steps && draggedStep >= 0 && draggedStep < n_stepseqsteps)
    {
        ...
        {
            yTop =
                sr.bottom - ((float)0.5f + sr.getHeight() * (0.5f + 0.5f * ss->steps[draggedStep]));
        }
...
CMouseEventResult CLFOGui::onMouseUp(CPoint &where, const CButtonState &buttons)
{
    ...
    if (controlstate == cs_trigtray_toggle)
    {
        ...
        for (int i = 0; i < n_stepseqsteps; i++)
        {
            ...
            {
                uint64_t off = 0, on = 0;
                off = UINT64_C(1) << i | UINT64_C(1) << (i + 16) | UINT64_C(1) << (i + 32);
    ...
    if (controlstate == cs_linedrag)
    {
        ...
        for (int i = 0; i < 16; ++i)
        {
            if (steprect[i].pointInside(rmStepStart))
                startStep = i;
            if (steprect[i].pointInside(rmStepCurr))
                endStep = i;
        };
...
CMouseEventResult CLFOGui::onMouseMoved(CPoint &where, const CButtonState &buttons)
{
    ...
    if (rect_shapes.pointInside(where))
    {
        // getFrame()->setCursor( VSTGUI::kCursorHand );
             ...
             (ss_shift_left.pointInside(where) || ss_shift_right.pointInside(where)))
    {
...
bool CLFOGui::onWheel(const VSTGUI::CPoint &where, const float &distance,
