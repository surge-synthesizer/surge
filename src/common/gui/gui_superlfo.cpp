...
void drawtri(CRect r, CDrawContext *context, int orientation)
{
	...
	int a = 0;
	if(orientation > 0) a = (endx-startx)-1;
...
void gui_superlfo::draw (CDrawContext *dc)
{	
	assert(lfodata);
	assert(storage);
	assert(ss);
   ...
   cdisurf->begin();
	...
	if(ss && lfodata->shape.val.i == ls_stepseq)
	{
		for(int i=0; i<n_stepseqsteps; i++)
		{
			...
			if(edit_trigmask) 
			{
				...
				if (ss->trigmask & (1<<i)) cdisurf->fillRect(gstep,0xff000000);
		...
		CRect linerect(rect_ls);	linerect.top = maindisp.top-size.top; linerect.right = linerect.left+1; cdisurf->fillRect(linerect,0x00087f00);
		linerect = rect_le;			linerect.top = maindisp.top-size.top; linerect.left = linerect.right-1; cdisurf->fillRect(linerect,0x00087f00);
	...
	else
	{
		...
		modulation_lfo *tlfo = new modulation_lfo();
		...
		for(int y=0; y<h2; y++) column_d[y] = 0;

		for(int x=boxo.left; x<boxo.right; x++)
		{		
			for(int y=0; y<h2; y++) column[y] = 0;	
         ...
         for (int y = 0; y < h2; y++)
         {
            cdisurf->setPixel(x, y, coltable[min((unsigned int)255,/*(y>>1) +*/ (column[y] >> aa_bs))]);
         }
		}
		delete tlfo;
	}
...
CMouseEventResult gui_superlfo::onMouseDown (CPoint &where, const CButtonState& buttons)
{
...
CMouseEventResult gui_superlfo::onMouseUp (CPoint &where, const CButtonState& buttons)
{
	if(controlstate)
	{
		//onMouseMoved(where,buttons);
...
CMouseEventResult gui_superlfo::onMouseMoved (CPoint &where, const CButtonState& buttons)
{
	...
	{
		ss->loop_start = limit_range((int)(where.x - steprect[0].left + (scale>>1))/scale,0,n_stepseqsteps-1);
		invalid();
	}
	else if(controlstate == cs_loopend)
	{
		ss->loop_end = limit_range((int)(where.x - steprect[0].left - (scale>>1))/scale,0,n_stepseqsteps-1);
		invalid();
	}
	else if(controlstate == cs_steps)
	{
		for(int i=0; i<n_stepseqsteps; i++)
		{
			if((where.x > steprect[i].left)&&(where.x < steprect[i].right))
			{
				...
				if(buttons & (kControl|kRButton)) f = 0;
				else if(lfodata->unipolar.val.b) f = limit_range(f,0.f,1.f);
				else f = limit_range(f * 2.f - 1.f,-1.f,1.f);
				if(buttons & kShift)
				{
					f *= 12;
					f = floor(f);
	...
	else if((controlstate == cs_trigtray_false)||(controlstate == cs_trigtray_true))
	{
		for(int i=0; i<n_stepseqsteps; i++)
		{
			if((where.x > gaterect[i].left)&&(where.x < gaterect[i].right))
			{
				...
				ss->trigmask = (ss->trigmask & minv) | (((controlstate == cs_trigtray_true)||(buttons & (kControl|kRButton)))?0:m);
				invalid();		
			}
		}
	}
