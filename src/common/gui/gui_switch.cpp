...
gui_switch::gui_switch(const CRect& size, IControlListener* listener, long tag, CBitmap* background)
...
void gui_switch::draw(CDrawContext* dc)
{
...
void gui_switch::setValue(float f)
{
   down = f > 0.5;
   value = down ? 1 : 0;
}

CMouseEventResult gui_switch::onMouseDown(CPoint& where, const CButtonState& buttons)
{
...
CMouseEventResult gui_switch::onMouseUp(CPoint& where, const CButtonState& buttons)
{
   return kMouseEventHandled;
}
CMouseEventResult gui_switch::onMouseMoved(CPoint& where, const CButtonState& buttons)
{
   return kMouseEventHandled;
}
/*
