...
class sub3_editor : public EditorType, public IControlListener, public IKeyboardHook
{
private:
   typedef EditorType super;
public:
	sub3_editor(void *effect, sub3_synth* synth);
	virtual ~sub3_editor ();
	virtual void idle ();
	bool queue_refresh;
...
protected:
   ...
   virtual int32_t onKeyDown(const VstKeyCode& code, CFrame* frame);	///< should return 1 if no further key down processing should apply, otherwise -1
   virtual int32_t onKeyUp(const VstKeyCode& code, CFrame* frame);	///< should return 1 if no further key up processing should apply, otherwise -1
...
private:
	...
	int fxbypass_tag = 0,resolink_tag = 0,f1resotag = 0,f1subtypetag = 0,f2subtypetag = 0,filterblock_tag = 0;
	void draw_infowindow(int ptag, CControl *control, bool modulate, bool forceMB=false);

   bool showPatchStoreDialog(patchdata *p, vector<patchlist_category> *patch_category, int startcategory);

   surge_bitmap_keeper bitmap_keeper;
