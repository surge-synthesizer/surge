...
CSurgeSlider::CSurgeSlider(const CPoint &loc, long stylee, IControlListener *listener, long tag,
    ...
    if (pHandle && showHandle && (modmode != 2) && (!isDeactivatedFn() || !disabled))
    {
        ...
        if (style & CSlider::kHorizontal)
        {
            ...
            if (pHandleHover && in_hover && (!modmode))
                pHandleHover->draw(dc, hrect, CPoint(0, 24 * typehy),
                                   modmode ? slider_alpha : slider_alpha);
        ...
        else if ((!isDeactivatedFn() || !disabled))
        {
            pHandle->draw(
                ...
                modmode ? slider_alpha
                        : slider_alpha); // used to be 0x80 which was solid - lets keep taht for now
            if (pHandleHover && in_hover && (!modmode))
                pHandleHover->draw(
                    dc, hrect, CPoint(0, 28 * typehy),
                    modmode
...
bool CSurgeSlider::isInMouseInteraction() { return controlstate == cs_drag; }
...
CMouseEventResult CSurgeSlider::onMouseUp(CPoint &where, const CButtonState &buttons)
{
    ...
    if (controlstate)
    {
        ...
        if (resetPosition && startPosition != currentPosition)
        {
...
VSTGUI::CMouseEventResult CSurgeSlider::onMouseEntered(VSTGUI::CPoint &where,
...
VSTGUI::CMouseEventResult CSurgeSlider::onMouseExited(VSTGUI::CPoint &where,
                                                      const VSTGUI::CButtonState &buttons)
{
    in_hover = false;
...
void CSurgeSlider::onMouseMoveDelta(const CPoint &where, const CButtonState &buttons, double dx,
                                    double dy)
{
    currentPosition = where;
...
void CSurgeSlider::onSkinChanged()
{
    if (style & CSlider::kHorizontal)
    {
        ...
        pTempoSyncHandle =
            associatedBitmapStore->getBitmapByStringID("TEMPOSYNC_HORIZONTAL_OVERLAY");
        pTempoSyncHoverHandle =
            associatedBitmapStore->getBitmapByStringID("TEMPOSYNC_HORIZONTAL_HOVER_OVERLAY");
    }
    else
    {
        ...
        pTempoSyncHoverHandle =
            associatedBitmapStore->getBitmapByStringID("TEMPOSYNC_VERTICAL_HOVER_OVERLAY");
    }
