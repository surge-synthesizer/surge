...
class gui_superlfo : public CControl
{
public:
	...
	gui_superlfo (const CRect &size, bool trigmaskedit, IControlListener *listener = 0, long tag = 0, sub3_lfo *lfodata=0, sub3_storage *storage=0, sub3_stepsequence* ss=0)
		: CControl(size,listener,tag,0)
	{
		...
		for(int i=0; i<256; i++)
		{
			...
			unsigned int r = limit_range((int)((float)255.f*powf(x*f_fgcol[0] + (1.f-x)*f_bgcol[0],1.f/2.2f)),0,255);			
			unsigned int g = limit_range((int)((float)255.f*powf(x*f_fgcol[1] + (1.f-x)*f_bgcol[1],1.f/2.2f)),0,255);			
			unsigned int b = limit_range((int)((float)255.f*powf(x*f_fgcol[2] + (1.f-x)*f_bgcol[2],1.f/2.2f)),0,255);			
         unsigned int a = 0xff;
...
protected:
	...
	CLASS_METHODS(gui_superlfo, CControl)
};