...
void COscillatorDisplay::openCustomEditor()
{
    ...
    struct AliasAdditive : public CustomEditor
    {
        ...
        std::array<CRect, n_alias_additive_partials> sliders;
        ...
        void draw(VSTGUI::CDrawContext *dc) override
        {
            ...
            divSize.top += 12;
            divSize.bottom -= 11;
            auto w = divSize.getWidth() / n_alias_additive_partials;

            dc->saveGlobalState();
            dc->setDrawMode(VSTGUI::kAntiAliasing | VSTGUI::kNonIntegralMode);

            for (int i = 0; i < n_alias_additive_partials; ++i)
            {
            ...
            if (buttons & kRButton)
            {
                ...
                }
                {
                    ...
                    auto action = [this](CCommandMenuItem *item) {
                        for (int qq = 0; qq < n_alias_additive_partials; ++qq)
                        {
                ...
                }
                {
                    ...
                    auto action = [this](CCommandMenuItem *item) {
                        float pdata[n_alias_additive_partials];
