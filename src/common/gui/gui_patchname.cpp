...
void gui_patchname::draw (CDrawContext*dc)
{	
...
CMouseEventResult gui_patchname::onMouseDown(CPoint &where, const CButtonState& button)
{
   ...
   char txt[256];
	...
	if(single_category) contextMenu->setNbItemsPerColumn(32);
	...
	{	
		...
		{
			if(!single_category && 
				((c == storage->patch_category_split[0]) ||
				(c == storage->patch_category_split[1]))) 
			...
			for(int subc=0; subc<n_subc; subc++)
			{
				char name[256];		
				COptionMenu *subMenu;
				if(single_category) subMenu = contextMenu;
            else
            {
            ...
            int sub = 0;				
               ...
               subMenu->addEntry(actionItem);
				...
				if(n_subc>1) sprintf(name,"%s - %i",storage->patch_category[c].name.c_str(),subc+1);			
				else strncpy(name,storage->patch_category[c].name.c_str(),namechars);
...
void gui_patchname::loadPatch(int id)
{
   if (listener && (id >= 0))
   {
      sel_id = id;
