...
void gui_hswitch::draw (CDrawContext*dc)
{
	if (getBackground())
	{
		// source position in bitmap
		CPoint where (0, heightOfOneImage * (long)(imgoffset + ((value * (float)(rows*columns - 1) + 0.5f))));
		getBackground()->draw(dc,getViewSize(),where,0xff);			
	}
	setDirty (false);
}

CMouseEventResult gui_hswitch::onMouseDown (CPoint &where, const CButtonState& buttons)
{
...
CMouseEventResult gui_hswitch::onMouseUp (CPoint &where, const CButtonState& buttons)
{
	if(dragable)
	{
		endEdit();
...
CMouseEventResult gui_hswitch::onMouseMoved (CPoint &where, const CButtonState& buttons)
{
