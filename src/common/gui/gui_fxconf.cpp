...
gui_fxconf::gui_fxconf (const CRect &size, IControlListener *listener, long tag, int current)
...
void gui_fxconf::draw (CDrawContext*dc)
{
...
CMouseEventResult gui_fxconf::onMouseDown(CPoint& where, const CButtonState& buttons)
{
	if (!buttons.isLeftButton() && !buttons.isRightButton()) return kMouseEventHandled;
	...
	for(int i=0; i<8; i++)
	{
      CRect size = getViewSize();
		...
		{
			if (buttons.isRightButton()) disabled ^= (1<<i);
			else current = i;
			invalid();
		}
	}
...
CMouseEventResult gui_fxconf::onMouseUp(CPoint & where, const CButtonState & buttons)
{
   return kMouseEventHandled;
}
