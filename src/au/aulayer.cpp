...
int aulayer::GetNumCustomUIComponents() { return 1; }
...
ComponentResult aulayer::Version() { return kVersionNumber; }
...
bool aulayer::HasIcon() { return true; }
...
CFURLRef aulayer::GetIconLocation()
{
    ...
    if (bundle)
    {
        ...
        if (resources)
        {
            icon = CFURLCreateCopyAppendingPathComponent(NULL, resources, CFSTR("surgeicon.icns"),
                                                         false);
            CFRelease(resources);
        }
    }
...
bool aulayer::IsPluginInitialized() { return plugin_instance != 0; }
...
void aulayer::Cleanup() { AUInstrumentBase::Cleanup(); }
...
ComponentResult aulayer::ChangeStreamFormat(AudioUnitScope inScope, AudioUnitElement inElement,
...
bool aulayer::ValidFormat(AudioUnitScope inScope, AudioUnitElement inElement,
                          const CAStreamBasicDescription &inNewFormat)
{
...
ComponentResult aulayer::StopNote(MusicDeviceGroupID inGroupID, NoteInstanceID inNoteInstanceID,
                                  UInt32 inOffsetSampleFrame)
{
    return 0;
}
...
OSStatus aulayer::HandleNoteOn(UInt8 inChannel, UInt8 inNoteNumber, UInt8 inVelocity,
                               UInt32 inStartFrame)
{
...
OSStatus aulayer::HandleNoteOff(UInt8 inChannel, UInt8 inNoteNumber, UInt8 inVelocity,
                                UInt32 inStartFrame)
{
...
OSStatus aulayer::HandleControlChange(UInt8 inChannel, UInt8 inController, UInt8 inValue,
                                      UInt32 inStartFrame)
{
...
OSStatus aulayer::HandlePitchWheel(UInt8 inChannel, UInt8 inPitch1, UInt8 inPitch2,
                                   UInt32 inStartFrame)
{
...
OSStatus aulayer::HandlePolyPressure(UInt8 inChannel, UInt8 inKey, UInt8 inValue,
                                     UInt32 inStartFrame)
{
                                ...
                                const AudioTimeStamp &inTimeStamp, UInt32 inNumberFrames)
{
    ...
    if (CallHostBeatAndTempo(&CurrentBeat, &CurrentTempo) >= 0)
    {
        ...
        // oscillator start and stop; but in performance mode we get freerun working off the
        // internal clock which we synthesize by running the "move clock" block below.
        //
...
ComponentResult aulayer::GetPropertyInfo(AudioUnitPropertyID iID, AudioUnitScope iScope,
                                         AudioUnitElement iElem, UInt32 &iSize, Boolean &fWritable)
{
                                          ...
                                          UInt32 &outNumParameters)
{
    ...
    if (outParameterList)
        memcpy(outParameterList, parameterIDlist,
               sizeof(AudioUnitParameterID) * (n_total_params + num_metaparameters));
    return noErr;
}
...
ComponentResult aulayer::GetParameter(AudioUnitParameterID inID, AudioUnitScope inScope,
...
ComponentResult aulayer::SetParameter(AudioUnitParameterID inID, AudioUnitScope inScope,
...
bool aulayer::ParameterBeginEdit(int p)
{
    ...
    event.mArgument.mParameter.mParameterID =
        p; // plugin_instance->remapInternalToExternalApiId(p);
...
//
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
OSStatus aulayer::HandleMidiEvent(UInt8 status, UInt8 channel, UInt8 data1, UInt8 data2,
                                  UInt32 inStartFrame)
{
    if (!IsInitialized())
        return kAudioUnitErr_Uninitialized;
