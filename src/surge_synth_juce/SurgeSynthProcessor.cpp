                       ...
                       )
{
   ...
   for(auto par: surge->storage.getPatch().param_ptr )
   {
      if( par )
      {
         ...
         // FIXME: use pm.clump to make as set of https://docs.juce.com/master/classAudioProcessorParameterGroup.html
...
//==============================================================================
const String SurgeSynthProcessor::getName() const
{
    return JucePlugin_Name;
}

bool SurgeSynthProcessor::acceptsMidi() const
{
    return false;
}

bool SurgeSynthProcessor::producesMidi() const
{
    return false;
}

bool SurgeSynthProcessor::isMidiEffect() const
{
    return false;
}

double SurgeSynthProcessor::getTailLengthSeconds() const
{
    return 2.0;
}
...
{
   if( surge->patchid < 0 || surge->patchid > surge->storage.patch_list.size() ) return 0;
...
{
   if( index == 0 ) return "INIT OR DROPPED";
   index --;
...
AudioProcessorEditor* SurgeSynthProcessor::createEditor()
{
...
{
    return new SurgeSynthProcessor();
}
