...
SurgeLv2Ui::SurgeLv2Ui(SurgeLv2Wrapper *instance, void *parentWindow,
    ...
    : _editor(new SurgeGUIEditor(instance, instance->synthesizer(), this)), _instance(instance),
      _writeFn(writeFn), _controller(controller), _uiScaleFactor(uiScaleFactor),
      _uiInitialized(false)
{
    ...
    if (uiResizer)
    {
        ...
        _editor->setZoomCallback([this, uiResizer](SurgeGUIEditor *editor, bool resizeWindow) {
            handleZoom(editor, uiResizer, resizeWindow);
        });
    }
    ...
    if (uiResizer)
    {
        ...
        uiResizer->ui_resize(uiResizer->handle, (rect->right - rect->left) * uiScaleFactor,
...
LV2UI_Handle SurgeLv2Ui::instantiate(const LV2UI_Descriptor *descriptor, const char *plugin_uri,
                                     ...
                                     const LV2_Feature *const *features)
{
    ...
    auto *featureOptions =
        (const LV2_Options_Option *)SurgeLv2::findFeature(LV2_OPTIONS__options, features);
    ...
    for (int i = 0; featureOptions[i].key != 0; ++i)
    {
        ...
        {
            if (featureOptions[i].type ==
                featureUridMap->map(featureUridMap->handle, LV2_ATOM__Float))
...
void SurgeLv2Ui::portEvent(LV2UI_Handle ui, uint32_t port_index, uint32_t buffer_size,
