...

    {
        ...
        for (unsigned pNth = 0; pNth < n_total_params; ++pNth)
        {
            ...
            osDsp << " [\n"
                  ...
                  << "\" ;\n"
                     "        lv2:default "
                  << std::fixed << std::setw(10) << std::setprecision(8)
                  << defaultSynth->getParameter01(did)
                  << " ;\n"
                     "        lv2:minimum "
                  << std::fixed << std::setw(10) << std::setprecision(8) << pMeta.fmin
                  << " ;\n"
                     "        lv2:maximum "
                  << std::fixed << std::setw(10) << std::setprecision(8) << pMeta.fmax
                  << " ;\n"
                     "    ]";
            ++portIndex;
        }
        ...
        // TODO LV2: implement an adequate version number scheme. For now, make it the last two
        // (so 1.6.2 gets 6 2)
        osDsp << "    lv2:minorVersion " << Surge::Build::SubVersionInt
              << " ;\n"
                 "    lv2:microVersion "
              << Surge::Build::ReleaseNumberStr << " .\n";
    }
