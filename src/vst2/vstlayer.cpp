...
{
	return new vstlayer (audioMaster);
}

//-------------------------------------------------------------------------------------------------------
vstlayer::vstlayer (audioMasterCallback audioMaster)
...
VstPlugCategory vstlayer::getPlugCategory ()
{ 
	if(plug_is_synth) return kPlugCategSynth; 
	else return kPlugCategEffect; 
}

void vstlayer::inputConnected (VstInt32 index, bool state)
{
...
{		
	...
	if(plugin_instance)
	{
		plugin_instance->~sub3_synth();
		_aligned_free(plugin_instance);
	}
}

VstInt32 vstlayer::vendorSpecific(VstInt32 lArg1, VstInt32 lArg2, void* ptrArg, float floatArg)
{
   if(lArg1 == 'MPEC')
   {
      if (lArg2 == 7)
...
VstInt32 vstlayer::stopProcess()
{
	if (initialized) plugin_instance->all_notes_off();
	return 1;
}
//-------------------------------------------------------------------------------------------------------
...
void vstlayer::init()
{
	...
	if(!synth)
	{
		// out of memory
#if WIN32
		MessageBox(::GetActiveWindow(), "Could not allocate memory.","Out of memory", MB_OK | MB_ICONERROR);
#endif

		return;
	}
	new(synth) sub3_synth(this); 	
	plugin_instance = (sub3_synth*)synth;

	plugin_instance->set_samplerate(this->getSampleRate());
   editor = new sub3_editor(this, plugin_instance);
...
VstInt32 vstlayer::canDo (char* text)
{
...
VstInt32 vstlayer::processEvents (VstEvents* ev)
{
	if(initialized)
   {				
...
void vstlayer::handleEvent(VstEvent* ev){
	...
	{		
		...
		{			
			if (midiData[1] == 0x7b || midiData[1] == 0x7e) 
				...
				plugin_instance->channel_controller((char)channel,midiData[1] & 0x7f,midiData[2] & 0x7f);
		}
...
//-------------------------------------------------------------------------------------------------------
void vstlayer::setProgramName (char *name)
{
	strcpy (programName, name);
}

//-----------------------------------------------------------------------------------------
void vstlayer::getProgramName (char *name)
{	
...
{	
	...
	//strncpy(currentProgram->name,s->storage.getPatch().name.c_str(),63);	currentProgram->name[63] = 0;
	strncpy(currentProgram->name,s->storage.patch_list.at(id).name.c_str(),63);	currentProgram->name[63] = 0;
	return id;
}
...
long vstlayer::getMidiProgramName (long channel, MidiProgramName* midiProgramName)
{
	...
	strncpy(midiProgramName->name,s->storage.patch_list.at(id).name.c_str(),63);	midiProgramName->name[63] = 0;
	midiProgramName->parentCategoryIndex = s->storage.patch_list.at(id).category;
	return n;
}
long vstlayer::getMidiProgramCategory(long channel,MidiProgramCategory *category)
{
	...
	{
		strncpy(category->name,s->storage.patch_category.at(id).name.c_str(),63);	category->name[63] = 0;		
	}
...
*/
//-----------------------------------------------------------------------------------------
void vstlayer::setParameter (VstInt32 index, float value)
{
	if (!initialized) init();	

	plugin_instance->setParameter01(plugin_instance->remapExternalApiToInternalId(index),value,true);
...
//-----------------------------------------------------------------------------------------
float vstlayer::getParameter (VstInt32 index)
{
	if (!initialized) init();	
...
//-----------------------------------------------------------------------------------------
void vstlayer::getParameterName (VstInt32 index, char *label)
{
	if (!initialized) init();	
...
//-----------------------------------------------------------------------------------------
void vstlayer::getParameterDisplay (VstInt32 index, char *text)
{
	if (!initialized) init();	
...
//-----------------------------------------------------------------------------------------
void vstlayer::getParameterLabel(VstInt32 index, char *label)
{
	strcpy (label, "");
}
/*
	...
	{		
		...
		for(int p = 0; p < n; p++)
		{
			if(s->storage.patch_list[p].category == category)
			{
				if(id == index) 
				{
					strncpy(text,s->storage.patch_list.at(p).name.c_str(),23);	text[23] = 0;
					return true;
				}
				else id++;			
			}
...
//------------------------------------------------------------------------
bool vstlayer::getEffectName (char* name)
{
#ifdef ISDEMO
...
//------------------------------------------------------------------------
bool vstlayer::getProductString (char* name)
{
#ifdef ISDEMO
	strcpy (name, "Surge demo");
#else
...
//------------------------------------------------------------------------
bool vstlayer::getVendorString (char* text)
{
...
template<bool replacing> void vstlayer::processT(float **inputs, float **outputs, VstInt32 sampleFrames)
{
	if (!initialized) return;
   ...
   _fpuState.set();
	...
	if(timeinfo)
	{
		if(timeinfo->flags & kVstTempoValid)	plugin_instance->time_data.tempo = timeinfo->tempo;	
		if(timeinfo->flags & kVstTransportPlaying)
		{
			if(timeinfo->flags & kVstPpqPosValid) plugin_instance->time_data.ppqPos = timeinfo->ppqPos;
		}
	}
	else
	{
...
//-----------------------------------------------------------------------------------------
void vstlayer::process (float **inputs, float **outputs, VstInt32 sampleFrames)
{
	processT<false>(inputs,outputs,sampleFrames);
}

//-----------------------------------------------------------------------------------------
void vstlayer::processReplacing (float **inputs, float **outputs, VstInt32 sampleFrames)
{    
...
bool vstlayer::getOutputProperties (VstInt32 index, VstPinProperties* properties)
{	
	if (index < 2)
	{
...
bool vstlayer::getInputProperties (VstInt32 index, VstPinProperties* properties)
{
...
void vstlayer::setSampleRate (float sampleRate)
{
...
VstInt32 vstlayer::getChunk (void** data, bool isPreset)
{
	if (!initialized) init();
...
VstInt32 vstlayer::setChunk (void* data, VstInt32 byteSize, bool isPreset)
{
	if (!initialized) init();

	if (byteSize <= 4) return 0;
