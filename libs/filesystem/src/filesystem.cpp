...
bool create_directories(const path &p)
{
    ...
    while (end && (end = std::strchr(end, path::preferred_separator)))
    {
        *end = '\0';
        if (!do_create_directory(begin) && !is_type_set_errno(begin, S_IFDIR))
            throw filesystem_error{ENOTDIR, std::string{"could not create directories \""} +
                                                p.native() + '\"'};
    ...
    throw filesystem_error{EEXIST,
                           std::string{"could not create directories \""} + p.native() + '\"'};
}

bool create_directory(const path &p) { return do_create_directory(p.c_str()); }
...
bool is_directory(const path &p) { return is_type(p, S_IFDIR); }

bool is_regular_file(const path &p) { return is_type(p, S_IFREG); }
