...
struct recursive_directory_iterator::Impl
{
    ...
    bool descend(const path &p)
    {
        ...
        throw filesystem_error{
            "recursive directory iterator cannot open directory \"" + p.native() + '\"', ec};
    }

    explicit Impl(directory_iterator &&it) { stack.push(std::move(it)); }
};
...
const recursive_directory_iterator::value_type &
