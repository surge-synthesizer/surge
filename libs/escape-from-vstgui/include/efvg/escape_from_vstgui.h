...
#define DUNIMPL std::cout << "Unimplemented : " << __func__ << " at " << __FILE__ << ":" << __LINE__ << std::endl;
...
struct CPoint {
   ...
   bool operator==(const CPoint &that) const
   {
   ...
   CPoint operator-(const CPoint &that ) {
      return CPoint( x - that.x, y - that.y );
   }
...
struct CRect
{
   ...
   CRect(const CPoint &corner, const CPoint &size) {
      top = corner.y; left=corner.x; right = left + size.x; bottom = top + size.y;
   }
   ...
   CRect centerInside( const CRect &r ) { UNIMPL; return r; }
   bool rectOverlap(const CRect &r ) { UNIMPL; return false; }
   inline void setHeight( float h ) {
      bottom = top + h;
   }
   inline void setWidth( float h ) {
      right = left + h;
   }
   ...
   operator juce::Rectangle<float>() const { return juce::Rectangle<float>(left,top, getWidth(), getHeight()); }
   juce::Rectangle<int> asJuceIntRect() const { juce::Rectangle<float> f = *this; return f.toNearestIntEdges();}
...
struct CColor
{
   ...
   constexpr CColor( uint8_t r, uint8_t g, uint8_t b, uint8_t a ) :red(r), green(g), blue(b), alpha(a) {}

   bool operator==(const CColor &that )
   {
      return red == that.red && green == that.green && blue == that.blue && alpha == that.alpha;
   }
   juce::Colour asJuceColour() const
   {
...
{
   CDrawMode() = default;
   CDrawMode(const CDrawModeFlags &f ) {
      UNIMPL;
   }
   CDrawMode(uint32_t x) {
      UNIMPL;
   }
};
...
struct CGraphicsTransform
{
   ...
   CGraphicsTransform inverse()
   {
      return CGraphicsTransform(juceT.inverted());
   }
...
struct CDrawContext
{
   ...
   void setDrawMode(uint32_t m) {
      UNIMPL;
   }
   void setDrawMode(const CDrawMode &m) {
      UNIMPL;
   }
   ...
   void fillLinearGradient(CGraphicsPath *p, const CGradient &, const CPoint &, const CPoint &, bool, CGraphicsTransform *tf ) {
      UNIMPL;
   }
   ...
   void getClipRect(CRect &r) const {
      UNIMPL;
   }
   void setClipRect(const CRect &r) {
   ...
   juce::Colour fillColor = juce::Colour(255,0,0),
                frameColor = juce::Colour(255,0,0),
                fontColor = juce::Colour( 255, 0, 0 );
   void setFontColor( const CColor &c ) {
      fontColor = c.asJuceColour();
   }
   CColor getFontColor() { UNIMPL; return CColor();  }

   void setFillColor(const CColor &c ) {
      fillColor = c.asJuceColour();
   }

   void setFrameColor(const CColor &c ) {
      frameColor = c.asJuceColour();
   }
   ...
   void drawPolygon( const std::vector<CPoint>&, CDrawStyle s)
   {
      UNIMPL;
   }
   ...
   void drawString( const char*, const CPoint &, int = 0, bool = true )
   {
      UNIMPL;
   }
...
template<typename T>
struct CViewWithJuceComponent;

template<typename T>
struct juceCViewConnector : public T
{
...
template<typename T>
struct CViewWithJuceComponent : public Internal::FakeRefcount
{
   ...
   void setViewSize(const CRect &r) {
   ...
   virtual bool magnify( CPoint &where, float amount ) {
      return false;
   }
   ...
   virtual bool onWheel(const CPoint &where, const CMouseWheelAxis &, const float &distance, const CButtonState &buttons )
   {
      UNIMPL;
      return false;
   }
   ...
   void invalid()
   {
      juceComponent()->repaint();
   }

   CRect ma;
   CRect getMouseableArea() {
      return ma;
   }
   void setMouseableArea(const CRect &r) {
      ma = r;
   }

   CPoint localToFrame( CPoint &w ) { UNIMPL; return w; }

   void setDirty(bool b = true)
   {
      if( b ) invalid();
   }
   bool isDirty() { UNIMPL; return true; }
   void setVisible( bool b ) { UNIMPL; }
   bool isVisible() { UNIMPL; return true; }
...
#define GSPAIR( x, sT, gT, gTV  ) \
   ...
   gT get ## x() const { UNIMPL; return gTV; }
...
struct CView : public CViewWithJuceComponent<juce::Component>{
   ...
   CView* getParentView() { UNIMPL; return nullptr; }
...
struct CViewContainer : public CView{
   ...
   CBitmap *bg = nullptr;
   void setBackground( CBitmap *b ) { if( bg ) bg->forget(); bg = b; bg->remember(); }
   void setBackgroundColor(const CColor &c) { UNIMPL; }
   int getNbViews() { UNIMPL; return 0; }
   CView *getView(int i) { UNIMPL; return nullptr; }
   ...
   bool isChild( CView *v ) { UNIMPL; return true; }
   CView *getFocusView() { UNIMPL; return nullptr; }
...
{
   CControl(const CRect &r, IControlListener *l = nullptr,
            int32_t tag = 0, CBitmap *bg = nullptr ) : CView(r), listener(l), tag(tag) {
      juceComponent()->setBounds(r.asJuceIntRect());
      setBackground(bg);
   }
   ~CControl()
   ...
   virtual float getRange() { UNIMPL; return 0; }
   ...
   virtual bool getMouseEnabled() { UNIMPL; return true; }
   ...
   void setBackColor(const CColor &c )
   {
      UNIMPL;
   }

   CBitmap *bg = nullptr;
   void setBackground( CBitmap *b ) { if( bg ) bg->forget(); bg = b; if( bg ) bg->remember(); }
   ...
   CGradient *getGradientHighlighted() { UNIMPL; return nullptr; }
   void setTextColor(const CColor &c ) { UNIMPL; }
   void setBackgroundColor(const CColor &c) { UNIMPL; }
   CColor getTextColor() { UNIMPL; return CColor(); }
   void setTextAlignment(CHoriTxtAlign c) { UNIMPL; }
   CGradient *getGradient() { UNIMPL; return nullptr; }
   CGradient *getGradientHover() { UNIMPL; return nullptr; }
   CColor getFrameColor() { UNIMPL; return CColor(); }
   CColor getFillColor() { UNIMPL; return CColor(); }
...
{
   CTextEdit( const CRect &r, IControlListener *l = nullptr, long tag = -1, const char* txt = nullptr) : CControl(r,l,tag) { UNIMPL; }
   GSPAIR(Text, const OfCourseItHasAStringType &, OfCourseItHasAStringType, "" );
...
{
   CCheckBox( const CRect &r, IControlListener *l = nullptr, long tag = -1, const char* txt = nullptr) : CControl(r,l,tag) { UNIMPL; }
...
{
   CTextButton( const CRect &r, IControlListener* l, int32_t tag, std::string lab ) : CControl(r,l,tag){
      UNIMPL;
   }
...
{
   // FIX THIS obviously
   CHorizontalSwitch(const CRect &size, IControlListener *listener, int32_t tag, int frames, int height, int width,
                     CBitmap *background, CPoint offset ) : CControl( size, listener, tag ), heightOfOneImage(height) {
...
struct CCommandMenuItem : public CMenuItem {
   ...
   void setActions( std::function<void(CCommandMenuItem *)> f, void *userData = nullptr) { func = f;}
...
struct COptionMenu : public CControl {
   ...
   COptionMenu( const CRect &r, IControlListener *l, int32_t tag, int, int = 0, int = 0 ) : CControl(r,l), menu() {
      UNIMPL;
   }
   ...
   CMenuItem *addEntry( COptionMenu *m,
                 const std::string &nm ) {
...
struct JuceVSTGUIEditorAdapter : public JuceVSTGUIEditorAdapterBase
{
   ...
   juce::AudioProcessorEditor *getJuceEditor() {
      return parentEd;
   }
...
struct JuceVSTGUIEditorAdapterConcreteTestOnly : public JuceVSTGUIEditorAdapter {
   JuceVSTGUIEditorAdapterConcreteTestOnly(juce::AudioProcessorEditor *parentEd ) : JuceVSTGUIEditorAdapter(parentEd) {}
